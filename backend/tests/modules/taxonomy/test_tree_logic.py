...
class TestTaxonomyTreeLogic:
    ...
    def test_orphan_node_handling(self, db_session, create_test_category):
        ...
        **Validates: Requirements 3.2, 3.3, 8.4**
        ...
        child_after_delete = (
            db_session.query(TaxonomyNode).filter_by(id=child_id).first()
        )
        ...
        if child_after_delete is None:
            ...
            assert (
                child_after_delete.parent_id is None
                or child_after_delete.parent_id != parent_id
            )
        ...
        **Validates: Requirements 3.2, 3.3, 8.4, 16.2**
        ...
        with pytest.raises((ValueError, IntegrityError, RuntimeError)) as exc_info:
            service.create_category(
                name="Beyond Max Depth", db=db_session, parent_id=current_parent_id
            )
    ...
    def test_empty_category_name(self, db_session):
        ...
        **Validates: Requirements 3.2, 3.3, 8.4, 16.2**
        ...
        with pytest.raises((ValueError, IntegrityError)):
            category = TaxonomyNode(name="", slug="empty", level=0, path="/empty")
        ...
        with pytest.raises((ValueError, IntegrityError)):
            category = TaxonomyNode(
                name="   ", slug="whitespace", level=0, path="/whitespace"
            )
            db_session.add(category)
            db_session.commit()
