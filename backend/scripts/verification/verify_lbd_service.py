...
def test_lbd_service_initialization():
    ...
    try:
        ...
        print(f"  - Graph embeddings service (lazy): {lbd_service._graph_embeddings_service is None}")
        ...
        print(f"✓ Graph embeddings service lazy loaded: {type(graph_embeddings_service).__name__}")
...
def test_open_discovery():
    ...
    try:
        ...
        stored_hypotheses = db.query(db_models.DiscoveryHypothesis).filter(
...
def test_closed_discovery():
    ...
    try:
        ...
        if paths:
            stored_hypothesis = db.query(db_models.DiscoveryHypothesis).filter(
...
def test_hypothesis_retrieval():
    ...
    try:
        ...
        # Query by type
        open_hypotheses = db.query(db_models.DiscoveryHypothesis).filter(
            db_models.DiscoveryHypothesis.hypothesis_type == "open"
        ).count()
        
        closed_hypotheses = db.query(db_models.DiscoveryHypothesis).filter(
            db_models.DiscoveryHypothesis.hypothesis_type == "closed"
        ).count()
        ...
        high_plausibility = db.query(db_models.DiscoveryHypothesis).filter(
            db_models.DiscoveryHypothesis.plausibility_score >= 0.7
        ).count()
        ...
        if all_hypotheses:
            ...
            print(f"    A Resource: {sample.a_resource.title if sample.a_resource else 'N/A'}")
            print(f"    C Resource: {sample.c_resource.title if sample.c_resource else 'N/A'}")
