...
def verify_classification_integration():
    ...
    try:
        with open('backend/app/services/resource_service.py', 'r', encoding='utf-8') as f:
            content = f.read()
    ...
    if 'from backend.app.services.classification_service import ClassificationService' in content:
    ...
    if 'Phase 8.5: ML Classification' in content:
        ...
        classification_block = content[classification_start:classification_start + 2000]
        
        if 'try:' in classification_block and 'except Exception as classification_exc:' in classification_block:
    ...
    if checks_passed == total_checks:
        print("✓ All checks passed! Classification integration is correctly implemented.")
        print()
...
def verify_requirements():
    ...
    requirements = [
        ...
        ("12.2", "Classification executes after embedding generation, before quality scoring"),
        ("12.3", "Classification failures handled gracefully without blocking ingestion")
    ]
