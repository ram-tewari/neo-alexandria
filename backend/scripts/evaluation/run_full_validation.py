...
class FullValidationRunner:
    ...
    def step2_validate_metrics(self) -> bool:
        ...
        args = [
            "--results-file",
            str(self.output_dir / "pipeline_results.json"),
            "--output-file",
            str(self.output_dir / "validation_report.json"),
        ]
    ...
    def step3_run_benchmarks(self) -> bool:
        ...
        args = [
            ...
            model_version,
            "--output-file",
            str(self.benchmark_dir / "benchmark_results.json"),
        ]
    ...
    def step4_generate_report(self) -> bool:
        ...
        args = [
            ...
            str(self.benchmark_dir / "benchmark_results.json"),
            "--output-file",
            "backend/docs/PRODUCTION_ML_TRAINING_RESULTS.md",
        ]
    ...
    def print_final_summary(self, results: dict, start_time: datetime):
        ...
        if all(results.values()):
            print("Next Steps:")
            print(
                "  1. Review the performance report: backend/docs/PRODUCTION_ML_TRAINING_RESULTS.md"
            )
            print(
                "  2. Check validation results: data/e2e_validation/validation_report.json"
            )
            print(
                "  3. Review benchmark results: data/benchmarks/benchmark_results.json"
            )
            print(
                "  4. If all requirements are met, proceed with production deployment"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--baseline-version", type=str, help="Baseline model version for comparison"
    )
    parser.add_argument(
        "--quick-test", action="store_true", help="Run quick test with only 1000 papers"
    )
    ...
    runner = FullValidationRunner(
        num_papers=num_papers, baseline_version=args.baseline_version
    )
