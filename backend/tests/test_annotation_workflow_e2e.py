...
class TestAnnotationWorkflowE2E:
    ...
    def test_complete_annotation_workflow(self, client: TestClient, db_session: Session):
        """
        ...
        for ann_data in annotation_data_list:
            response = client.post(f"/resources/{str(resource_id)}/annotations", json=ann_data)
            assert response.status_code in [200, 201], f"Annotation creation failed: {response.status_code} - {response.text}"
        ...
        response = client.get("/annotations/search/fulltext", params={"query": "learning"})
        assert response.status_code == 200
        search_results = response.json()
        assert len(search_results) >= 2  # Should find multiple annotations with "learning"
        ...
        response = client.get("/annotations/search/semantic", params={
        ...
        if semantic_results:
            ...
            print(f"✓ Step 4: Semantic search found {len(semantic_results)} annotations")
        else:
        ...
        print(f"✓ Step 8: Retrieved {len(resource_annotations)} annotations for resource")
