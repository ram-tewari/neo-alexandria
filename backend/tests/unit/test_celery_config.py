...
class TestCeleryConfiguration:
    ...
    def test_task_routing_rules(self):
        ...
        assert routes["app.tasks.celery_tasks.regenerate_embedding_task"]["queue"] == "high_priority"
        
        assert "app.tasks.celery_tasks.update_search_index_task" in routes
        assert routes["app.tasks.celery_tasks.update_search_index_task"]["queue"] == "urgent"
        
        assert "app.tasks.celery_tasks.classify_resource_task" in routes
        assert routes["app.tasks.celery_tasks.classify_resource_task"]["queue"] == "ml_tasks"
        
        assert "app.tasks.celery_tasks.batch_process_resources_task" in routes
        assert routes["app.tasks.celery_tasks.batch_process_resources_task"]["queue"] == "batch"
...
class TestCeleryBeatSchedule:
    ...
    def test_quality_degradation_monitoring_schedule(self):
        ...
        assert schedule["task"] == "app.tasks.celery_tasks.monitor_quality_degradation_task"
    ...
    def test_classification_retraining_schedule(self):
        ...
        assert schedule is not None
        assert schedule["task"] == "app.tasks.celery_tasks.retrain_classification_model_task"
