...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'reverse_migration_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
...
class PostgreSQLToSQLiteMigrator:
    ...
    def __init__(self, source_url: str, target_url: str, batch_size: int = 1000, dry_run: bool = False):
        ...
        logger.warning("This is a REVERSE migration - some PostgreSQL features will be lost")
    ...
    def validate_schemas(self) -> bool:
        ...
        if missing_in_target:
            ...
            logger.error("Please run Alembic migrations on target SQLite database first")
            return False
        ...
        for table_name in self.TABLE_ORDER:
            ...
            for col in skip_columns:
                if col in source_columns:
                    logger.info(f"Table '{table_name}': skipping PostgreSQL-only column '{col}'")
            ...
            if missing_columns:
                ...
                self.warnings.append(f"Table '{table_name}': missing columns {missing_columns}")
    ...
    def convert_row_data(self, row_dict: Dict[str, Any], table_name: str, source_columns: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            while offset < stats['source_count']:
                try:
                    ...
                    batch_query = select(source_table).limit(self.batch_size).offset(offset)
                    ...
                    for row in batch_rows:
                        ...
                        converted_row = self.convert_row_data(row_dict, table_name, source_columns)
            ...
            if stats['success']:
                logger.info(f"Table '{table_name}': reverse migration completed successfully")
            else:
...
Examples:
  ...
  python migrate_postgresql_to_sqlite.py \\
    ...
    parser.add_argument(
        '--source',
        required=True,
        help='Source database URL (PostgreSQL)'
    )
    parser.add_argument(
        '--target',
        required=True,
        help='Target database URL (SQLite)'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
