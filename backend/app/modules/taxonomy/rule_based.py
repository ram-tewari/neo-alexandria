...
class RuleBasedClassifier:
    ...
    def classify(self, resource_id: str, top_k: int = 5) -> List[Dict]:
        """
    ...
    def _compute_rule_score(self, content: str, keywords: List[str]) -> float:
        """
    ...
    def _find_node_by_name(self, node_name: str) -> TaxonomyNode:
        ...
        """
        node = (
            self.db.query(TaxonomyNode).filter(TaxonomyNode.name == node_name).first()
        )

        return node

    def add_rule(
        self, rule_id: str, node_name: str, keywords: List[str], confidence: float = 0.8
    ) -> None:
        """
