...
class TestCollaborativeFilteringMetrics:
    ...
    def test_ndcg_at_10(
        self, trained_ncf_model, recommendation_test_data: Dict[str, Any]
    ):
        """
        ...
        for user_id, test_items in user_test_cases.items():
            ...
            try:
                ...
                pred_scores = [
                    predictions.get(item_id, 0.0) for item_id in candidate_ids
                ]
    ...
    def test_cold_start_performance(
        ...
        print(
            f"Cold start users (<{cold_start_threshold} interactions): {len(cold_start_users)}"
        )
        ...
        success_rate = (
            successful_predictions / total_cold_start_users
            if total_cold_start_users > 0
            else 0.0
        )
        ...
        cold_start_interaction_counts = [
            user_interaction_counts[user_id] for user_id in cold_start_test_cases.keys()
        ]
