...
class ArxivCollector:
    ...
    def __init__(self, output_dir: str = "data/raw/arxiv"):
        ...
        self.client = arxiv.Client(page_size=100, delay_seconds=0.5, num_retries=3)

        logger.info(
            f"ArxivCollector initialized with output directory: {self.output_dir}"
        )
        ...
        try:
            ...
            for result in self.client.results(search):
                try:
                    ...
                    if count % 100 == 0:
                        logger.info(
                            f"Collected {count}/{max_results} papers from {category}"
                        )
            ...
            if papers:
                self._save_intermediate_results(
                    category, papers, len(papers), error=True
                )
    ...
    def _save_intermediate_results(
        self, category: str, papers: List[Dict], count: int, error: bool = False
    ):
        """
        Save intermediate results to prevent data loss.
    ...
    def collect_balanced_dataset(
        ...
        logger.info(
            f"Papers per category: {[len(papers) for papers in dataset.values()]}"
        )
