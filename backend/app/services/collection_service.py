...
- app/routers/collections.py: API endpoints that use this service
- app/services/ai_core.py: Embedding generation for collections
...
class CollectionService:
    ...
    def get_collection_resources(
        ...
        query = self.db.query(Resource).join(
            CollectionResource,
            Resource.id == CollectionResource.resource_id
        ).filter(
            CollectionResource.collection_id == collection_id
        )
        ...
        if filters:
            ...
            if filters.get("classification_code"):
                query = query.filter(Resource.classification_code == filters["classification_code"])
        ...
        resources = query.order_by(
            CollectionResource.added_at.desc()
