...
class HybridRecommendationService:
        ...
        try:
            ...
            if strategy in ["collaborative", "hybrid"] and use_collaborative:
                try:
                    ...
                    # Get NCF predictions
                    ncf_recommendations = self.collaborative_filtering_service.get_top_recommendations(
                        user_id=str(user_id),
                        candidate_ids=resource_ids,
                        limit=100
                    )
                    ...
                    logger.info(f"Generated {len(ncf_recommendations)} collaborative candidates for user {user_id}")
                
                except Exception as e:
                    logger.warning(f"Error generating collaborative candidates: {str(e)}")
            
            # 2. Content-Based Candidates
            if strategy in ["content", "hybrid"]:
                try:
                    # Get user embedding
                    user_embedding = self.user_profile_service.get_user_embedding(user_id)
                    ...
                    for resource in resources:
                        ...
                        try:
                            ...
                            similarity = self._cosine_similarity(user_embedding, np.array(resource_emb))
                            ...
                            logger.debug(f"Error computing similarity for resource {resource.id}: {str(e)}")
                            continue
                    
                    # Sort by content score and take top 100
                    content_candidates.sort(key=lambda x: x['content_score'], reverse=True)
                    ...
                    logger.info(f"Generated {len(content_candidates)} content candidates for user {user_id}")
            ...
            if strategy in ["graph", "hybrid"] and self.graph_service is not None:
                try:
                    ...
                    ).order_by(desc(UserInteraction.interaction_timestamp)).limit(10).all()
                    ...
                    for interaction in recent_interactions:
                        ...
                        for neighbor in neighbors:
                            try:
                                ...
                                if neighbor_id not in candidate_ids_seen and neighbor_id not in graph_candidate_ids:
                                ...
                                logger.debug(f"Error processing graph neighbor: {str(e)}")
                                continue
                    
                    # Limit to top 100 graph candidates
                    graph_candidates = [c for c in candidates if c['source_strategy'] == 'graph']
                    graph_candidates.sort(key=lambda x: x.get('graph_score', 0), reverse=True)
                    graph_candidates = graph_candidates[:100]
                    
                    # Update candidates list
                    candidates = [c for c in candidates if c['source_strategy'] != 'graph'] + graph_candidates
                    
                    # Update seen IDs
                    for candidate in graph_candidates:
                        candidate_ids_seen.add(candidate['resource_id'])
                    
                    logger.info(f"Generated {len(graph_candidates)} graph candidates for user {user_id}")
                
                except Exception as e:
                    logger.warning(f"Error generating graph candidates: {str(e)}")
            elif strategy in ["graph", "hybrid"]:
                logger.debug("Graph-based recommendations currently disabled for module isolation")
            
            logger.info(f"Total candidates generated: {len(candidates)} for user {user_id}")
            return candidates
        
        except Exception as e:
            logger.error(f"Error in _generate_candidates for user {user_id}: {str(e)}", exc_info=True)
            return []
    ...
    def _rank_candidates(
        ...
        try:
            ...
            for candidate in candidates:
                ...
                quality_score = resource.quality_overall if resource.quality_overall else 0.5
                ...
                if resource.publication_year:
                    ...
                    if year_range > 0:
                        recency_score = max(0.0, min(1.0, (resource.publication_year - 1900) / year_range))
            ...
            logger.info(f"Ranked {len(scored_candidates)} candidates for user {user_id}")
            return scored_candidates
        
        except Exception as e:
            logger.error(f"Error in _rank_candidates for user {user_id}: {str(e)}", exc_info=True)
            return []
        ...
        try:
            ...
            for candidate in candidates:
                ...
                try:
                    ...
                    logger.debug(f"Error parsing embedding for resource {resource.id}: {str(e)}")
                    continue
            
            # Handle case where no valid embeddings
            if not valid_candidates:
                logger.warning("No valid embeddings for MMR, returning top candidates by score")
            ...
            while len(selected) < limit and remaining_indices:
                ...
                for idx in remaining_indices:
                    ...
                    for selected_emb in selected_embeddings:
                        similarity = self._cosine_similarity(candidate_emb, selected_emb)
            ...
            logger.info(f"MMR selected {len(selected)} diverse candidates (lambda={lambda_param})")
            return selected
    ...
    def _apply_novelty_boost(
        ...
        try:
            ...
            view_count_results = self.db.query(
                UserInteraction.resource_id,
                func.count(UserInteraction.id).label('count')
            ).filter(
                UserInteraction.resource_id.in_(resource_ids),
                UserInteraction.interaction_type == 'view'
            ).group_by(UserInteraction.resource_id).all()
            
            # Create view counts dict
            view_counts = {resource_id: count for resource_id, count in view_count_results}
            ...
            if novel_count < min_novel:
                ...
                novel_candidates = [c for c in boosted_candidates if c['novelty_score'] > 0.5]
            ...
            logger.info(f"Applied novelty boosting: {novel_count}/{total_candidates} novel candidates (threshold={novelty_preference})")
    ...
    def generate_recommendations(
        ...
        try:
            ...
            if is_cold_start and strategy == "collaborative":
                logger.info(f"Cold start user {user_id} (interactions={interaction_count}), switching to content+graph")
            ...
            if filters:
                ...
                for candidate in ranked_candidates:
                    ...
                    quality = resource.quality_overall if resource.quality_overall else 0.5
                ...
                logger.info(f"Quality filtering: {len(ranked_candidates)} candidates remaining")
            
            # Step 4: Apply MMR diversity optimization
            mmr_start = time.time()
            diversified_candidates = self._apply_mmr(ranked_candidates, profile, limit * 2)  # Get 2x for novelty boost
            mmr_time = time.time() - mmr_start
            
            # Step 5: Apply novelty boosting
            novelty_start = time.time()
            boosted_candidates = self._apply_novelty_boost(diversified_candidates, profile)
            ...
            logger.error(f"Error in generate_recommendations for user {user_id}: {str(e)}", exc_info=True)
            return {
                'recommendations': [],
                'metadata': {
                    'total': 0,
