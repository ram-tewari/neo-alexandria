...
def test_hybrid_recommendation_fusion():
    ...
    ranked_resources = sorted(final_scores.items(), key=lambda x: x[1], reverse=True)
...
def test_recommendation_generation_flow(
    client, db_session, mock_event_bus, create_test_resource
):
    """
    ...
    try:
        response = client.post(
            ...
            json={"user_id": "test_user", "limit": 10, "strategy": "hybrid"},
        )
