...
def upgrade() -> None:
    ...
    if not table_exists('ab_test_experiments'):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(['control_version_id'], ['model_versions.id'], ondelete='CASCADE'),
            sa.ForeignKeyConstraint(['treatment_version_id'], ['model_versions.id'], ondelete='CASCADE'),
            sa.PrimaryKeyConstraint('id')
        )
    ...
    if not index_exists('ab_test_experiments', 'idx_ab_test_experiments_dates'):
        op.create_index('idx_ab_test_experiments_dates', 'ab_test_experiments', ['start_date', 'end_date'])
    ...
    if not index_exists('prediction_logs', 'idx_prediction_logs_experiment_id'):
        op.create_index('idx_prediction_logs_experiment_id', 'prediction_logs', ['experiment_id'])
    if not index_exists('prediction_logs', 'idx_prediction_logs_model_version_id'):
        op.create_index('idx_prediction_logs_model_version_id', 'prediction_logs', ['model_version_id'])
...
def downgrade() -> None:
    ...
    if table_exists('prediction_logs'):
        ...
        if index_exists('prediction_logs', 'idx_prediction_logs_model_version_id'):
            op.drop_index('idx_prediction_logs_model_version_id', table_name='prediction_logs')
        if index_exists('prediction_logs', 'idx_prediction_logs_experiment_id'):
            op.drop_index('idx_prediction_logs_experiment_id', table_name='prediction_logs')
    ...
    if table_exists('ab_test_experiments'):
        if conn.dialect.name == 'postgresql':
            op.drop_constraint('ck_ab_test_experiments_traffic_split_range', 'ab_test_experiments', type_='check')
        
        if index_exists('ab_test_experiments', 'idx_ab_test_experiments_dates'):
            op.drop_index('idx_ab_test_experiments_dates', table_name='ab_test_experiments')
        if index_exists('ab_test_experiments', 'idx_ab_test_experiments_status'):
            op.drop_index('idx_ab_test_experiments_status', table_name='ab_test_experiments')
