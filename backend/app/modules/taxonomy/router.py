...
@taxonomy_router.post("/categories", response_model=CategoryResponse)
def create_category(data: CategoryCreate, db: Session = Depends(get_sync_db)):
    """
...
def get_predictions(resource_id: str, db: Session = Depends(get_sync_db)) -> Dict:
    """
...
def retrain_model(
    ...
    **Validates: Requirements 9.5, 9.6, 9.7, 9.10**
    ...
    try:
        ...
        training_tuples = [
            (sample["resource_id"], sample["node_id"]) for sample in samples
        ]
...
@taxonomy_router.get("/uncertain", response_model=UncertaintyResponse)
def get_uncertain_predictions(
    threshold: float = 0.5, limit: int = 100, db: Session = Depends(get_sync_db)
):
    """
