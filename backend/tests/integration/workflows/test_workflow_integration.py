...
def test_update_resource_triggers_quality_recomputation(db_session, sample_resource):
    ...
    with patch('backend.app.services.quality_service.QualityService') as mock_quality_service:
        ...
        updated_resource = update_resource(db_session, sample_resource.id, update_payload)
        
        # Verify quality service was called
        mock_quality_service.assert_called_once()
        mock_service_instance.compute_quality.assert_called_once_with(sample_resource.id)
...
def test_update_resource_no_quality_recomputation_for_non_affecting_fields(db_session, sample_resource):
...
def test_scheduled_outlier_detection(db_session, sample_resource):
    ...
    with patch('backend.app.services.scheduled_tasks.QualityService') as mock_quality_service:
        mock_service_instance = Mock()
        ...
        # Verify quality service was called
        mock_service_instance.detect_quality_outliers.assert_called_once_with(batch_size=100)
...
def test_scheduled_degradation_monitoring(db_session, sample_resource):
    ...
    with patch('backend.app.services.scheduled_tasks.QualityService') as mock_quality_service:
        ...
        # Verify quality service was called
        mock_service_instance.monitor_quality_degradation.assert_called_once_with(time_window_days=30)
