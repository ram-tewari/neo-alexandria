...
def test_create_minimal_resource():
    ...
    resource = create_minimal_resource(
        title="Test Resource", quality_score=0.9, language="en"
    )
...
def test_create_minimal_resources_batch():
    ...
    resources = create_minimal_resources_batch(
        count=10, base_quality=0.7, quality_increment=0.05, language="en"
    )
...
def test_create_resource_with_quality_score():
    ...
    resource = create_resource_with_quality_score(
        "Test Resource", quality, language="en"
    )
...
def test_create_minimal_authority_subject():
    ...
    subject = create_minimal_authority_subject("Machine Learning", usage_count=100)
...
def test_cleanup_test_data(test_db):
    ...
    assert (
        db.query(AuthoritySubject).filter(AuthoritySubject.id.in_(subject_ids)).count()
        == 3
    )

    # Cleanup all at once
    cleanup_test_data(db, (Resource, resource_ids), (AuthoritySubject, subject_ids))
