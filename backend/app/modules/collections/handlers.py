...
def handle_resource_deleted(event):
    ...
    try:
        ...
        try:
            ...
            stmt = select(CollectionResource).where(
            ...
            logger.info(
                f"Removed resource {resource_id} from {len(associations)} collections"
            )
...
def emit_collection_resource_added(collection_id: str, resource_id: str, user_id: str):
    ...
    try:
        ...
        logger.debug(
            f"Emitted collection.resource_added event for collection {collection_id}"
        )
    except Exception as e:
        logger.error(
            f"Error emitting collection.resource_added event: {str(e)}", exc_info=True
        )


def emit_collection_resource_removed(
    collection_id: str,
    resource_id: str,
    user_id: str = None,
    ...
    try:
        ...
        event_bus.emit(
            "collection.resource_removed", payload, priority=EventPriority.NORMAL
        )
        logger.debug(
            f"Emitted collection.resource_removed event for collection {collection_id}"
        )
    except Exception as e:
        logger.error(
            f"Error emitting collection.resource_removed event: {str(e)}", exc_info=True
        )
