...
class TestSummarizationQualityMetrics:
    ...
    def test_bertscore(
        self,
        summarization_test_data: Dict[str, Any],
        mock_openai_api
    ):
        """
        ...
        try:
            ...
            pytest.skip("bert-score package not installed. Install with: pip install bert-score")
    ...
    def test_rouge_scores(
        ...
        try:
            ...
            pytest.skip("rouge-score package not installed. Install with: pip install rouge-score")
        ...
        print("-" * 80)
        print(f"{'ROUGE-1':<15} {avg_rouge1:>10.4f} {np.min(rouge1_scores):>10.4f} "
              f"{np.max(rouge1_scores):>10.4f} {np.std(rouge1_scores):>10.4f}")
        print(f"{'ROUGE-2':<15} {avg_rouge2:>10.4f} {np.min(rouge2_scores):>10.4f} "
              f"{np.max(rouge2_scores):>10.4f} {np.std(rouge2_scores):>10.4f}")
        print(f"{'ROUGE-L':<15} {avg_rougeL:>10.4f} {np.min(rougeL_scores):>10.4f} "
    ...
    def test_compression_ratio(
        ...
        print("\nDistribution:")
        print(f"  Below 5%:   {sum(1 for r in compression_ratios if r < 0.05)} samples "
              f"({sum(1 for r in compression_ratios if r < 0.05)/len(compression_ratios)*100:.1f}%)")
        print(f"  5-15%:      {sum(1 for r in compression_ratios if 0.05 <= r <= 0.15)} samples "
              f"({sum(1 for r in compression_ratios if 0.05 <= r <= 0.15)/len(compression_ratios)*100:.1f}%)")
        print(f"  Above 15%:  {sum(1 for r in compression_ratios if r > 0.15)} samples "
        ...
        if in_range:
            ...
            if avg_compression < min_target:
                ...
                print("  Recommendation: Generate longer summaries to capture more information")
            else:
