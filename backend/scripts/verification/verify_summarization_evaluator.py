...
spec = importlib.util.spec_from_file_location(
    ...
    os.path.join(
        os.path.dirname(__file__), "app", "services", "summarization_evaluator.py"
    ),
)
...
def test_summarization_evaluator():
    ...
    try:
        ...
        conciseness_optimal = evaluator.finesure_conciseness(
            summary_optimal, reference_optimal
        )
        ...
        conciseness_short = evaluator.finesure_conciseness(
            summary_short, reference_short
        )
        ...
        print("\n[TEST 8] Test error handling - resource not found")
        try:
            evaluator.evaluate_summary(resource_id=str(uuid.uuid4()), use_g_eval=False)
        ...
        result_no_summary = evaluator.evaluate_summary(
            resource_id=resource_no_summary.id, use_g_eval=False
        )
