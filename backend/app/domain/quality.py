...
class ReadabilityScore(ValueObject):
    ...
    def validate(self) -> None:
        ...
        if self.flesch_reading_ease < -50 or self.flesch_reading_ease > 150:
            raise ValueError(
                f"flesch_reading_ease {self.flesch_reading_ease} outside reasonable range"
            )

        # Gunning Fog typically ranges from 6-17
        if self.gunning_fog_index < 0 or self.gunning_fog_index > 30:
            raise ValueError(
                f"gunning_fog_index {self.gunning_fog_index} outside reasonable range"
            )
        ...
        if self.sentence_count < 0:
            raise ValueError(
                f"sentence_count {self.sentence_count} must be non-negative"
            )
        if self.avg_words_per_sentence < 0:
            raise ValueError(
                f"avg_words_per_sentence {self.avg_words_per_sentence} must be non-negative"
            )
        ...
        if self.paragraph_count < 0:
            raise ValueError(
                f"paragraph_count {self.paragraph_count} must be non-negative"
            )
...
class QualityScore(ValueObject):
    ...
    def has_dimension_below_threshold(self, threshold: float = 0.5) -> bool:
        ...
        """
        return any(score < threshold for score in self.get_dimension_scores().values())
    ...
    def count_dimensions_below_threshold(self, threshold: float = 0.5) -> int:
        ...
        """
        return sum(
            1 for score in self.get_dimension_scores().values() if score < threshold
        )
