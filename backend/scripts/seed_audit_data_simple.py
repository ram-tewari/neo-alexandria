"""
Simplified Seeding Script for Performance Audit
Uses only dense embeddings (Nomic) to avoid large model downloads.
...
DATASET = [
    {
        ...
        "annotations": [
            ...
            "Library for concurrent programming in Python"
        ]
    },
    {
        "cluster": "CODING",
...
def generate_synthetic_data():
    print("=" * 60)
    print("SEEDING AUDIT DATA (SIMPLIFIED)")
    print("=" * 60)
    ...
    for item in DATASET:
        ...
        # Serialize embedding as JSON string for SQLite
        embedding_json = json.dumps(dense_vec)

        # Create Resource
        res = Resource(
            title=item["title"],
            description=item["content"],  # Full content in description
            embedding=embedding_json,  # Store as JSON string
            subject=[item["cluster"]]  # Store cluster in subject field
        )
        ...
        for idx, note in enumerate(item["annotations"]):
            # Create simple annotations with offsets
            start = idx * 100
            end = start + len(note)
            ann = Annotation(
                resource_id=res.id,
    ...
    print("")
    print("NOTE: This simplified version uses only dense embeddings.")
    print("      Sparse embeddings (for hybrid search) are not included.")
    print("      Vector search will work, but hybrid search may be limited.")
    print("")
