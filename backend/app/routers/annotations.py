"""
Neo Alexandria 2.0 - Annotation API Endpoints (Phase 7.5)
...
- app/schemas/annotation.py: Request/response schemas
- app/database/models.py: Annotation and Resource models
"""
...
@router.post("/resources/{resource_id}/annotations", response_model=AnnotationResponse, status_code=201)
async def create_annotation(
    ...
    try:
        ...
        return AnnotationResponse(
            ...
            collection_ids=json.loads(annotation.collection_ids) if annotation.collection_ids else None,
...
@router.get("/resources/{resource_id}/annotations", response_model=AnnotationListResponse)
async def list_resource_annotations(
    resource_id: str,
    include_shared: bool = Query(False, description="Include shared annotations from other users"),
    ...
    try:
        ...
        items = [
            AnnotationResponse(
                ...
                collection_ids=json.loads(ann.collection_ids) if ann.collection_ids else None,
...
async def search_annotations_by_tags(
    ...
    match_all: bool = Query(False, description="If true, match ALL tags; if false, match ANY tag"),
    user_id: str = Depends(_get_current_user_id),
    service: AnnotationService = Depends(_get_annotation_service)
):
    """
...
async def export_annotations_markdown(
    resource_id: Optional[str] = Query(None, description="Optional resource ID to filter by"),
    user_id: str = Depends(_get_current_user_id),
    service: AnnotationService = Depends(_get_annotation_service)
):
    """
