...
class TestQualityComputationLatency:
    ...
    def test_single_resource_latency(self, quality_service, db_session):
        ...
        assert latency < 1.0, (
            f"Quality computation took {latency:.2f}s, expected < 1.0s"
        )

    def test_average_latency_multiple_resources(
        self, quality_service, create_test_resources
    ):
...
class TestBatchQualityComputationThroughput:
    ...
    def test_batch_throughput_100_resources(
        self, quality_service, create_test_resources
    ):
        ...
        assert throughput >= 100, (
            f"Throughput {throughput:.1f} resources/min, expected >= 100"
        )

    def test_batch_computation_scales_linearly(
        self, quality_service, create_test_resources
    ):
...
class TestOutlierDetectionPerformance:
    ...
    def test_outlier_detection_1000_resources(
        self, quality_service, create_test_resources
    ):
        ...
        assert elapsed_time < 30.0, (
            f"Outlier detection took {elapsed_time:.2f}s, expected < 30s"
        )

    def test_outlier_detection_with_batching(
        self, quality_service, create_test_resources
    ):
...
class TestSummarizationEvaluationLatency:
    ...
    def test_evaluation_without_g_eval_latency(
...
class TestDegradationMonitoringPerformance:
    ...
    def test_degradation_monitoring_performance(
        self, quality_service, create_test_resources
    ):
...
class TestMemoryUsage:
    ...
    def test_outlier_detection_memory_efficiency(
        self, quality_service, create_test_resources
    ):
...
class TestConcurrentOperations:
    ...
    def test_concurrent_quality_computations(
        self, quality_service, create_test_resources
    ):
