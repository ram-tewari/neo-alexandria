...
def test_pipeline_detects_new_data_correctly(temp_dirs):
    ...
    with patch.object(
        pipeline.arxiv_collector, "collect_papers_by_category", return_value=new_papers
    ):
...
def test_dataset_augmentation_combines_data_correctly(temp_dirs):
    ...
    assert total_samples <= len(existing_data) + len(
        new_papers
...
def test_new_version_training_works(temp_dirs):
    ...
    with patch(
        "scripts.training.retrain_pipeline.ClassificationTrainer"
    ) as MockTrainer:
...
def test_improvement_evaluation_compares_models_correctly(temp_dirs):
    ...
    with patch(
        ...
        prod_trainer.evaluate.return_value = {"accuracy": 0.90, "f1": 0.88}

        # New model
        new_trainer = MagicMock()
        new_trainer.evaluate.return_value = {"accuracy": 0.93, "f1": 0.91}
        ...
        comparison = pipeline.evaluate_improvement(
            new_version="v1.1.0", production_version="v1.0.0", test_samples=test_samples
        )
    ...
    assert (
        abs(comparison["improvement"]["accuracy"] - 0.03) < 0.001
...
def test_promotion_logic_works_when_improvement_exceeds_threshold(temp_dirs):
    ...
    with patch.object(pipeline.versioning, "promote_to_production"):
        promoted = pipeline.promote_if_better(
            new_version="v1.1.0", comparison=comparison
        )
