...
def test_quality_degradation_monitoring():
    ...
    try:
        ...
        recent_date = now - timedelta(days=10)  # 10 days ago (inside 30-day window)
        ...
        db.commit()
        print(f"  Simulated degradation: removed creator, year, DOI, authors")
        ...
        print(f"  Computed: {old_date.strftime('%Y-%m-%d')} (45 days ago)")
        print(f"  No changes made (stable)")
        ...
        print(f"  Should be SKIPPED (within 30-day window)")
        ...
        print(f"  No quality score computed")
        print(f"  Should be SKIPPED (no previous score)")
        ...
        if len(degraded_resources) > 0:
            ...
            for i, report in enumerate(degraded_resources, 1):
                ...
                print(f"   ✓ Degradation percentage calculated correctly")
        else:
        ...
        if quality_drop1 > 0.2:
            ...
            print(f"  ✓ Correctly flagged for review (needs_quality_review=True)")
            assert resource1.id in [r['resource_id'] for r in degraded_resources]
            print(f"  ✓ Included in degradation report")
        else:
            print(f"  ⚠ Quality drop below threshold (not flagged)")
        ...
        if quality_change2 <= 0.2:
            print(f"  ✓ Correctly NOT flagged (stable quality)")
            assert resource2.id not in [r['resource_id'] for r in degraded_resources]
            print(f"  ✓ Not included in degradation report")
        ...
        print(f"\nResource 3:")
        print(f"  ✓ Correctly skipped (recent computation)")
        print(f"  ✓ Not included in degradation report")
        ...
        print(f"\nResource 4:")
        print(f"  ✓ Correctly skipped (no previous quality score)")
        print(f"  ✓ Not included in degradation report")
