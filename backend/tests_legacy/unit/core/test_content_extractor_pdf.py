...
class TestPDFExtraction:
    ...
    def test_extract_pdf_with_pdfminer_fallback(self):
        ...
        with (
            patch("backend.app.utils.content_extractor.fitz", None),
            patch(
                "backend.app.utils.content_extractor.pdfminer_extract_text"
            ) as mock_pdfminer,
        ):
    ...
    def test_extract_pdf_pymupdf_error_fallback(self):
        ...
        with (
            patch("backend.app.utils.content_extractor.fitz") as mock_fitz,
            patch(
                "backend.app.utils.content_extractor.pdfminer_extract_text"
            ) as mock_pdfminer,
        ):
...
class TestContentTypeDetection:
    ...
    def test_extract_from_fetched_pdf_by_content_type(self):
        ...
        with patch(
            "backend.app.utils.content_extractor.extract_pdf"
        ) as mock_extract_pdf:
