...
class TestCollectionConstraints:
    ...
    def test_duplicate_resource_addition(
        ...
        **Validates: Requirements 5.3, 5.4, 8.4**
        ...
        added_count_1 = service.add_resources_to_collection(
            collection_id=collection.id, resource_ids=[resource.id], owner_id="user123"
        )
    ...
    def test_empty_collection_operations(self, db_session, create_test_collection):
        ...
        **Validates: Requirements 5.3, 5.4, 8.4**
        ...
        collection = create_test_collection(name="Empty Collection", owner_id="user123")
        ...
        resources, total = service.get_collection_resources(
            collection_id=collection.id, owner_id="user123"
        )
    ...
    def test_max_nesting_depth_prevention(self, db_session, create_test_collection):
        ...
        **Validates: Requirements 5.3, 5.4, 8.4**
        ...
        parent = create_test_collection(name="Parent Collection", owner_id="user123")

        # Create child collection
        child = create_test_collection(
            name="Child Collection", owner_id="user123", parent_id=parent.id
        )
    ...
    def test_nonexistent_resource_addition(self, db_session, create_test_collection):
        ...
        **Validates: Requirements 5.3, 5.4, 8.4**
        ...
        assert len(associations) == 0, (
            "No associations should be created for non-existent resources"
        )
    ...
    def test_access_control_enforcement(self, db_session, create_test_collection):
        ...
        **Validates: Requirements 5.3, 5.4**
        ...
        collection = create_test_collection(
            name="Private Collection", owner_id="user1", visibility="private"
        )
        ...
        # Owner can access
        result = service.get_collection(collection_id=collection.id, owner_id="user1")
        assert result is not None, "Owner should be able to access their collection"

        # Non-owner cannot access private collection
        result = service.get_collection(collection_id=collection.id, owner_id="user2")
        assert result is None, (
            "Non-owner should not be able to access private collection"
        )
