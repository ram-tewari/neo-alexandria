...
from typing import Generator, Union, List, Dict, Tuple, Any
...
def seeded_resources(test_db):
    ...
    try:
        db.query(Resource).filter(Resource.id.in_([r.id for r in items])).delete(
...
def recommendation_test_data(test_db):
    ...
    # Cleanup with bulk delete (faster than individual deletes)
    try:
        db.query(Resource).filter(Resource.id.in_([r.id for r in resources])).delete(
            synchronize_session=False
        )
        db.query(AuthoritySubject).filter(
            AuthoritySubject.id.in_([s.id for s in authority_subjects])
        ...
        class MockDDGS:
            ...
            def text(self, query, max_results):
                ...
                if "machine learning" in query.lower() or "ml" in query.lower():
                    ...
                    "ai" in query.lower() or "artificial intelligence" in query.lower()
                ):
...
def single_resource_library(test_db):
    ...
    subject = AuthoritySubject(
        canonical_form="Test Subject", variants=["test", "testing"], usage_count=1
    )
    db.add(subject)
...
def classification_result_factory():
    ...
    def _create(
        ...
        if predictions is None:
            predictions = [
                ClassificationPrediction(taxonomy_id="004", confidence=0.9, rank=1)
            ]
...
def recommendation_factory():
    ...
    def _create(
        ...
        recommendation_score = RecommendationScore(
            score=score, confidence=confidence, rank=rank
        )
...
def create_classification_service_mock(result=None):
    ...
    if result is None:
        result = ClassificationResult(
            predictions=[
                ClassificationPrediction(taxonomy_id="004", confidence=0.9, rank=1),
                ClassificationPrediction(taxonomy_id="006", confidence=0.75, rank=2),
            ],
    ...
    if recommendations is None:
        recommendations = [
            Recommendation(
                ...
                user_id="test-user",
                recommendation_score=RecommendationScore(
                    score=0.9, confidence=0.85, rank=1
                ),
                strategy="hybrid",
            ...
            Recommendation(
                ...
                user_id="test-user",
                recommendation_score=RecommendationScore(
                    score=0.8, confidence=0.75, rank=2
                ),
...
def assert_classification_result(
    ...
    if min_confidence is not None or max_confidence is not None:
        for i, pred in enumerate(predictions):
            confidence = (
                pred.confidence if hasattr(pred, "confidence") else pred["confidence"]
            )
    ...
    if check_high_confidence:
        high_conf_predictions = [
            p
            for p in predictions
            if (p.confidence if hasattr(p, "confidence") else p["confidence"])
...
def assert_search_result(
    ...
    if max_score is not None:
        assert score <= max_score, f"Expected score <= {max_score}, got {score}"
    ...
    if max_rank is not None:
        assert rank <= max_rank, f"Expected rank <= {max_rank}, got {rank}"
...
def assert_recommendation(
    ...
    if check_high_quality:
        is_high_quality = (
            score >= score_threshold and confidence >= confidence_threshold
        )
...
def assert_confidence_in_range(
    confidence: float, min_confidence: float = 0.0, max_confidence: float = 1.0
):
    """
...
def assert_results_sorted_by_score(
    results: List[Union[Tuple, Dict]], descending: bool = True
):
    """
...
def assert_search_result_presence(
    results: List[Union[str, Dict]], expected_ids: List[str]
):
    """
...
def mock_ml_classification_service():
    ...
    mock_service.fine_tune = Mock(
        return_value={"f1": 0.85, "precision": 0.87, "recall": 0.83}
    )
...
def mock_sparse_embedding_service():
    ...
    def mock_generate_sparse_embedding(text):
        ...
        return {100: 0.8, 200: 0.6, 300: 0.4, 400: 0.3, 500: 0.2}

    mock_service.generate_sparse_embedding = Mock(
        side_effect=mock_generate_sparse_embedding
    )
    ...
    mock_service.generate_sparse_embeddings_batch = Mock(
        side_effect=mock_generate_batch
    )

    # Configure update methods
    mock_service.update_resource_sparse_embedding = Mock(return_value=True)
    mock_service.batch_update_sparse_embeddings = Mock(
        return_value={"total": 10, "success": 10, "failed": 0, "skipped": 0}
    )

    # Configure search method
    def mock_search_by_sparse_vector(query_sparse, limit=10):
        # Return mock search results
        return [("resource-1", 0.95), ("resource-2", 0.85), ("resource-3", 0.75)][
            :limit
        ]

    mock_service.search_by_sparse_vector = Mock(
        side_effect=mock_search_by_sparse_vector
    )
...
def mock_ml_models_in_unit_tests(request):
    ...
    if is_unit_test and not use_real_models:
        with patch(
            "backend.app.services.ml_classification_service.MLClassificationService._load_model"
        ):
            with patch(
                "backend.app.services.sparse_embedding_service.SparseEmbeddingService._ensure_loaded",
                return_value=False,
            ):
                yield
    else:
