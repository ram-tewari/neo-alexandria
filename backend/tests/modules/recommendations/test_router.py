...
from app.database.models import Resource, UserProfile, UserInteraction, RecommendationFeedback
...
def test_track_interaction_success(client, db_session, test_user):
    ...
    # Verify in database
    interaction = db_session.query(UserInteraction).filter(
        UserInteraction.resource_id == res_id
    ).first()
...
def test_submit_feedback_success(client, db_session, test_user):
    ...
    # Verify in database
    feedback = db_session.query(RecommendationFeedback).filter(
        RecommendationFeedback.resource_id == res_id
    ).first()
