...
class TestPostgreSQLJSONB:
    ...
    def test_jsonb_containment_operator(self, db_session, db_type):
        ...
        results = (
            ...
            .filter(Resource.subject.op("@>")(cast(["Machine Learning"], JSONB)))
            .all()
        )
        ...
        results = (
            db_session.query(Resource)
            .filter(Resource.subject.op("@>")(cast(["Python"], JSONB)))
            .all()
        )
    ...
    def test_jsonb_key_exists_operator(self, db_session, db_type):
        ...
        results = (
            db_session.query(Resource).filter(Resource.relation.op("?")("cites")).all()
        )
        ...
        results = (
            db_session.query(Resource)
            .filter(Resource.relation.op("?")("cited_by"))
            .all()
        )
        ...
        results = (
            db_session.query(Resource)
            .filter(Resource.relation.op("?")("references"))
            .all()
        )
    ...
    def test_jsonb_array_length(self, db_session, db_type):
        ...
        results = (
            db_session.query(Resource)
            .filter(text("jsonb_array_length(subject) > 2"))
            .all()
        )
    ...
    def test_jsonb_concatenation(self, db_session, db_type):
        ...
        db_session.execute(
            text(
                "UPDATE resources SET subject = subject || :new_subject WHERE id = :id"
            ),
...
class TestPostgreSQLJSONBEmbeddings:
    ...
    def test_sparse_embedding_jsonb_storage(self, db_session, db_type):
        ...
        sparse_embedding = {"100": 0.8, "200": 0.6, "300": 0.4, "400": 0.3, "500": 0.2}
        ...
        results = (
            db_session.query(Resource)
            .filter(Resource.sparse_embedding.op("?")("100"))
            .all()
        )
