...
class CollaborativeFilteringService:
    ...
    def __init__(self, db: Session, model_path: str = "models/ncf_model.pt"):
        ...
        logger.info(f"CollaborativeFilteringService initialized with device: {self.device}")
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Loaded NCF model from {self.model_path} (users={num_users}, items={num_items})")
            return True
    ...
    def train_model(self, epochs: int = 10, batch_size: int = 256, learning_rate: float = 0.001) -> Dict:
        """
        ...
        try:
            ...
            if len(interactions) < 10:
                logger.warning(f"Insufficient training data: {len(interactions)} interactions")
                return {"error": "Insufficient training data", "num_interactions": len(interactions)}
            ...
            self.user_id_to_idx = {user_id: idx for idx, user_id in enumerate(sorted(unique_users))}
            self.item_id_to_idx = {item_id: idx for idx, item_id in enumerate(sorted(unique_items))}
