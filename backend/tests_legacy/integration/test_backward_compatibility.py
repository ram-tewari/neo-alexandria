...
def test_collections_endpoints_exist(client):
    ...
    assert response.status_code in [200, 404, 500], (
        "Collections list endpoint should exist"
    )
    ...
    assert response.status_code in [200, 201, 422, 500], (
        "Collections create endpoint should exist"
    )
...
def test_resources_endpoints_exist(client):
    ...
    assert response.status_code in [200, 201, 422], (
        "Resources create endpoint should exist"
    )
...
def test_search_endpoints_exist(client):
    ...
    assert response.status_code in [200, 422], (
        "Three-way hybrid search endpoint should exist"
    )
...
def test_response_schemas_unchanged(client):
    ...
    if response.status_code == 200:
        ...
        assert "items" in data or isinstance(data, list), (
            "Collections response should have items"
        )
    ...
    if response.status_code == 200:
        ...
        # Should have items and total
        assert "items" in data or isinstance(data, list), (
            "Resources response schema is compatible"
        )
