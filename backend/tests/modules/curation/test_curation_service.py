...
def test_batch_review_invalid_action(
    client: TestClient,
    create_test_resource
):
    """
...
def test_batch_tag_resources(
    ...
    batch_request = {
        ...
        "tags": ["new_tag", "another_new_tag", "NEW_TAG"]  # Duplicate in different case
    }
...
def test_assign_curator(
    ...
    # Create resources
    resources = [
        create_test_resource(title=f"Resource {i}")
        for i in range(5)
    ]
...
def test_enhanced_review_queue_by_status(
    client: TestClient,
    create_test_resource
):
    """
...
@pytest.mark.skip(reason="SQLite :memory: database isolation issue - resources created in test session not visible to API after first request commits")
...
def test_enhanced_review_queue_quality_range(
    client: TestClient,
    create_test_resource
):
    """
...
def test_batch_review_performance(
    client: TestClient,
    create_test_resource
):
    """
...
def test_review_queue_pagination_with_filters(
    client: TestClient,
    create_test_resource
):
    """
