...
class CanaryDeployment:
    ...
    def gradual_rollout(self) -> bool:
        ...
        try:
            ...
            for stage_percentage in self.rollout_stages:
                ...
                monitor_interval = min(60, self.stage_duration_seconds)  # Check every minute
                elapsed = 0
                ...
                while elapsed < self.stage_duration_seconds:
                    ...
                    if not self.metrics_acceptable():
                        ...
                        self._log_event("metrics_degraded", {
                            "percentage": stage_percentage,
                    ...
                    logger.info(f"  Elapsed: {elapsed}s / {self.stage_duration_seconds}s - Metrics OK")
            ...
            try:
                ...
                logger.info(f"Version {self.canary_version} promoted to production in registry")
            ...
            self._log_event("rollout_failed", {
                "version": self.canary_version,
                "error": str(e)
            })
            self.rollback()
    ...
    def _update_traffic_split(self, canary_percentage: int) -> None:
        ...
        logger.info(f"  Production ({self.production_version}): {production_percentage}%")
    ...
    def metrics_acceptable(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"  Error rate: {canary_error_rate:.4f} vs {production_error_rate:.4f}")
            logger.debug(f"  Latency p95: {canary_latency:.2f}ms vs {production_latency:.2f}ms")
    ...
    def rollback(self) -> bool:
        ...
        logger.warning(f"All traffic routed back to production: {self.production_version}")
...
def main():
    ...
    rollout_parser.add_argument('--canary-version', required=True, help='Version to deploy')
    rollout_parser.add_argument('--production-version', help='Current production version (optional)')
    rollout_parser.add_argument('--stage-duration', type=int, default=3600, 
                                help='Stage duration in seconds (default: 3600 = 1 hour)')
    
    # Rollback command
    rollback_parser = subparsers.add_parser('rollback', help='Rollback canary deployment')
    rollback_parser.add_argument('--production-version', help='Version to rollback to (optional)')
    
    # Check metrics command
    metrics_parser = subparsers.add_parser('metrics', help='Check current metrics')
    metrics_parser.add_argument('--canary-version', required=True, help='Canary version')
    metrics_parser.add_argument('--production-version', help='Production version (optional)')
