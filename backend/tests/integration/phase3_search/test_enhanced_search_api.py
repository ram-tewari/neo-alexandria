...
class TestEnhancedSearchEndpoint:
    ...
    def test_phrase_search(self, client, search_test_data):
        ...
        # Should find resources with the phrase (may be 0 if FTS5 not initialized)
    ...
    def test_boolean_search(self, client, search_test_data):
        ...
        # Should find resources with both terms (may be 0 if FTS5 not initialized)
        assert "total" in data
        assert "items" in data
        if data["total"] > 0:
            for item in data["items"]:
        ...
        # Should find resources with either term (may be 0 if FTS5 not initialized)
        assert "total" in data
        assert "items" in data
    ...
    def test_wildcard_search(self, client, search_test_data):
        ...
        assert response.status_code == 200
        data = response.json()
        # May be 0 if FTS5 not initialized
        assert "total" in data
        ...
        # If results found, verify they contain "learn"
        if data["total"] > 0:
            for item in data["items"]:
                snippet = data["snippets"][str(item["id"])]
