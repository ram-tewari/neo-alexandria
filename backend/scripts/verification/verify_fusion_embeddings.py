...
def test_fusion_embeddings():
    ...
    try:
        # Check prerequisites
        resources_with_content = db.query(Resource).filter(
            Resource.embedding.isnot(None)
        ).count()
        
        embeddings_with_structural = db.query(GraphEmbedding).filter(
            GraphEmbedding.structural_embedding.isnot(None)
        ).count()
        
        logger.info(f"Prerequisites: {resources_with_content} resources with content embeddings")
        logger.info(f"Prerequisites: {embeddings_with_structural} resources with structural embeddings")
        
        if resources_with_content == 0 or embeddings_with_structural == 0:
            logger.warning("⚠️  Need resources with both content and structural embeddings")
        ...
        fusion_count = db.query(GraphEmbedding).filter(
        ...
        sample_fusion = db.query(GraphEmbedding).filter(
            GraphEmbedding.fusion_embedding.isnot(None)
        ).first()
        ...
        if sample_fusion:
            ...
            assert 0.99 <= norm <= 1.01, f"Fusion vector should be normalized, got norm={norm}"
...
def test_hnsw_index_building():
    ...
    try:
        ...
        embeddings_with_index_id = db.query(GraphEmbedding).filter(
            GraphEmbedding.hnsw_index_id.isnot(None)
        ).count()
        
        logger.info(f"✓ {embeddings_with_index_id} embeddings have HNSW index IDs")
        assert embeddings_with_index_id == stats['total_embeddings'], "All embeddings should have index IDs"
...
def test_hnsw_index_querying():
    ...
    try:
        ...
        if results:
            ...
            logger.info("✓ Query resource correctly identified as most similar to itself")
...
def test_dimension_mismatch_handling():
    ...
    try:
        ...
        query = db.query(Resource, GraphEmbedding).join(
            GraphEmbedding,
            Resource.id == GraphEmbedding.resource_id
        ).filter(
            Resource.embedding.isnot(None),
            GraphEmbedding.structural_embedding.isnot(None)
        ).limit(1)
        ...
        if content_dim != structural_dim:
            logger.info(f"✓ Dimension mismatch detected ({content_dim} vs {structural_dim})")
            ...
            if graph_embedding.fusion_embedding:
                ...
                logger.info("✓ Fusion embedding correctly truncated to minimum dimension")
        else:
            logger.info(f"✓ Embeddings have same dimension ({content_dim})")
            logger.info("  (Dimension mismatch handling would work if dimensions differed)")
