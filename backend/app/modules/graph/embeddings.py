...
class GraphEmbeddingsService:
    ...
    def _build_networkx_graph(self):
        ...
        try:
            ...
            raise ImportError("NetworkX is required for graph embeddings. Install with: pip install networkx")
    ...
    def _generate_random_walks(self, G, num_walks: int, walk_length: int, p: float = 1.0, q: float = 1.0) -> List[List[str]]:
        """
    ...
    def _node2vec_walk(self, G, start_node: str, walk_length: int, p: float, q: float) -> List[str]:
        """
    ...
    def _biased_choice(self, G, cur: str, prev: str, neighbors: List[str], p: float, q: float) -> str:
        """
    ...
    def compute_node2vec_embeddings(
        ...
        try:
            ...
            raise ImportError("gensim is required for graph embeddings. Install with: pip install gensim")
        ...
        logger.info(f"Node2Vec embeddings computed in {execution_time:.2f}s for {len(embeddings)} nodes")
    ...
    def _store_embeddings(self, embeddings: Dict[str, List[float]], algorithm: str = "node2vec") -> None:
        """
