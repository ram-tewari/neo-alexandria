...
def check_model_health(
    ...
    if checkpoint_path:
        ...
        if checkpoint_dir.exists():
            ...
            if not missing_files:
                ...
                results["details"]["checkpoint_valid"] = (
                    f"All required files present in {checkpoint_path}"
                )
    ...
    if model is not None and test_input is not None:
        try:
            ...
            if hasattr(model, "predict"):
                ...
                raise AttributeError(
                    "Model does not have predict() or __call__() method"
                )
            ...
            results["details"]["inference_working"] = (
                f"Inference successful (latency: {latency_ms:.2f}ms)"
            )
            ...
            if latency_ms < latency_threshold_ms:
                ...
                results["details"]["latency_acceptable"] = (
                    f"Latency {latency_ms:.2f}ms < threshold {latency_threshold_ms}ms"
                )
                logger.info(
                    f"✓ Latency is acceptable: {latency_ms:.2f}ms < {latency_threshold_ms}ms"
                )
            else:
                results["details"]["latency_acceptable"] = (
                    f"Latency {latency_ms:.2f}ms >= threshold {latency_threshold_ms}ms"
                ...
                logger.warning(
                    f"✗ Latency is too high: {latency_ms:.2f}ms >= {latency_threshold_ms}ms"
                )
        ...
        if model is None:
            results["details"]["inference_working"] = (
                "Cannot test inference: model is None"
            )
            logger.info("⊘ Inference check skipped (model is None)")
        else:
            results["details"]["inference_working"] = (
                "Cannot test inference: no test input provided"
            )
...
def check_classification_model_health(
    ...
    if hasattr(service, "checkpoint_dir"):
        ...
        if checkpoint_path.exists():
            ...
            if not missing_files:
                results["checkpoint_valid"] = True
                results["details"]["checkpoint_valid"] = (
                    "All required checkpoint files present"
                )
    ...
    try:
        ...
        if isinstance(prediction, dict) and "label" in prediction:
            results["inference_working"] = True
            results["details"]["inference_working"] = (
                f"Classification successful: {prediction.get('label')}"
            )
        ...
        if latency_ms < latency_threshold_ms:
            results["latency_acceptable"] = True
            results["details"]["latency_acceptable"] = (
                f"Latency {latency_ms:.2f}ms < {latency_threshold_ms}ms"
            )
