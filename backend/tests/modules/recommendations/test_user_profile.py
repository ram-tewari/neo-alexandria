...
class TestInteractionTracking:
    ...
    def test_track_collection_add_interaction(self, db_session, test_user, test_resource):
    ...
    def test_duplicate_interaction_updates_return_visits(self, db_session, test_user, test_resource):
    ...
    def test_interaction_updates_profile_total(self, db_session, test_user, test_resource):
...
class TestProfileComputation:
    ...
    def test_interaction_counts_by_type(self, db_session, test_user, test_resource):
        ...
        assert counts.get('view', 0) >= 1  # At least one view (duplicates update same record)
...
class TestColdStart:
    ...
    def test_cold_start_profile(self, db_session, test_user):
        ...
        assert profile['interest_vector'] is None or all(v == 0 for v in profile['interest_vector'])
...
class TestProfileSettings:
    ...
    def test_update_profile_validation(self, db_session, test_user):
        ...
        with pytest.raises(ValueError, match="diversity_preference must be between 0.0 and 1.0"):
...
def test_resource(db_session):
    ...
    resource = Resource(
        title="Test Resource",
        description="Test content for user profile testing"
    )
    db_session.add(resource)
    db_session.commit()
    return resource
