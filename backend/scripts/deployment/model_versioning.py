...
class ModelVersioning:
    ...
    def __init__(self, base_dir: str = "models/classification"):
        ...
        logger.info(f"Model versioning initialized with base directory: {self.base_dir}")
        
        # Load existing version registry or create new one
        self.registry = self._load_or_create_registry()
        
        logger.info(f"Version registry loaded: {len(self.registry.get('versions', []))} versions")
    ...
    def load_version(self, version: str) -> Tuple[Any, Dict[str, Any]]:
        ...
        if version_entry is None:
            available_versions = [v['version'] for v in self.registry.get('versions', [])]
            raise ValueError(
        ...
        try:
            ...
            if label_map_file.exists():
                ...
                logger.info(f"Label mapping loaded: {len(label_map.get('label_to_id', {}))} classes")
    ...
    def promote_to_production(self, version: str) -> None:
        ...
        if previous_production:
            for v in self.registry['versions']:
                if v['version'] == previous_production:
                    v['status'] = 'archived'
                    logger.info(f"Previous production version {previous_production} archived")
                    break
        ...
        if production_link.exists() or production_link.is_symlink():
            if production_link.is_symlink():
                ...
                logger.warning(f"Production path exists as directory: {production_link}")
        ...
        try:
            ...
            logger.info(f"Created production symlink: {production_link} -> {version_dir}")
        except OSError as e:
...
def main():
    ...
    create_parser.add_argument('--model-path', required=True, help='Path to trained model')
    create_parser.add_argument('--version', required=True, help='Version string (e.g., v1.0.0)')
    create_parser.add_argument('--notes', default='', help='Optional notes about this version')
    ...
    compare_parser.add_argument('--test-data', required=True, help='Path to test data JSON')
