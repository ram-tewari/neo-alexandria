...
def verify_three_way_hybrid_search():
    ...
    try:
        ...
        # Long query
        analysis = AdvancedSearchService._analyze_query(
            "How does gradient descent work in deep neural networks for image classification"
        )
        ...
        query = SearchQuery(
            text="test query", filters=SearchFilters(), limit=20, offset=0
        )

        results = AdvancedSearchService.search_three_way_hybrid(
            db, query, enable_reranking=False, adaptive_weighting=True
        )
        ...
        sparse_results = AdvancedSearchService._search_sparse(
            db, "test query", limit=10
        )
