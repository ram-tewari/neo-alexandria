...
def load_checkpoint_with_validation(checkpoint_path: Path) -> Dict[str, Any]:
    ...
    if not checkpoint_path.exists():
        raise CheckpointValidationError(f"Checkpoint directory not found: {checkpoint_path}")
    ...
    try:
        ...
        if not isinstance(state_dict, dict):
            raise CheckpointValidationError("Invalid checkpoint format: state_dict is not a dictionary")
...
def validate_labels(samples: List[Dict[str, Any]], expected_labels: Optional[set] = None) -> Tuple[bool, Optional[str]]:
    """
...
def validate_dataset(samples: List[Dict[str, Any]], dataset_name: str = "dataset") -> None:
    """
    ...
    if not is_valid:
        ...
        raise DataValidationError(f"{dataset_name} label validation failed: {error_msg}")
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if logs:
            ...
            if "eval_accuracy" in logs:
                self.writer.add_scalar("Accuracy/validation", logs["eval_accuracy"], step)
...
class ClassificationTrainer:
    ...
    def load_datasets(self, data_dir: str) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        train_samples = train_data if isinstance(train_data, list) else train_data.get('samples', [])
        val_samples = val_data if isinstance(val_data, list) else val_data.get('samples', [])
        test_samples = test_data if isinstance(test_data, list) else test_data.get('samples', [])
...
class ArxivClassificationDataset(Dataset):
    ...
    def train(
        ...
        try:
            ...
            if "out of memory" in error_msg or "oom" in error_msg:
                ...
                logger.warning("Clearing GPU cache and retrying with reduced batch size...")
                ...
                try:
                    ...
                    if "out of memory" in error_msg2 or "oom" in error_msg2:
                        logger.error("Out of memory error persists even with reduced batch size")
                        logger.error("Suggestions:")
                        logger.error(f"  1. Further reduce batch size (try {reduced_batch_size // 2})")
                        logger.error("  2. Use a smaller model (e.g., distilbert-base-uncased)")
                        logger.error("  3. Reduce max sequence length (try 256 instead of 512)")
                        logger.error("  4. Use gradient accumulation to simulate larger batches")
                        raise
                    else:
                        raise
            else:
    ...
    def _train_with_batch_size(
        ...
        try:
            ...
            writer.add_graph(self.model, (sample_batch["input_ids"], sample_batch["attention_mask"]))
        ...
        if torch.cuda.is_available():
            gpu_memory_allocated = torch.cuda.memory_allocated(0) / (1024**3)
            gpu_memory_reserved = torch.cuda.memory_reserved(0) / (1024**3)
            logger.info(f"GPU memory before training - Allocated: {gpu_memory_allocated:.2f} GB, Reserved: {gpu_memory_reserved:.2f} GB")
        ...
        if torch.cuda.is_available():
            ...
            logger.info(f"GPU memory after training - Allocated: {gpu_memory_allocated:.2f} GB, Reserved: {gpu_memory_reserved:.2f} GB")
    ...
    def evaluate(self, test_samples: List[Dict]) -> Dict[str, Any]:
        ...
        if self.model is None or self.tokenizer is None:
            raise ValueError("Model and tokenizer must be initialized before evaluation")
