...
def verify_multihop_neighbors():
    ...
    try:
        ...
        if G.number_of_nodes() == 0:
            logger.warning("⚠ No nodes in graph. Please ensure resources exist in database.")
            return False
        ...
        resource = db.query(db_models.Resource).filter(
            db_models.Resource.id == UUID(test_resource_id)
        ).first()
        ...
        if neighbors_2hop:
            ...
            for i, neighbor in enumerate(neighbors_2hop[:3], 1):
                logger.info(f"    {i}. {neighbor['title']}")
                logger.info(f"       Distance: {neighbor['distance']}")
                logger.info(f"       Path: {' -> '.join(neighbor['path'][:2])}... -> {neighbor['path'][-1]}")
                logger.info(f"       Path strength: {neighbor['path_strength']:.3f}")
                logger.info(f"       Edge types: {neighbor['edge_types']}")
                logger.info(f"       Score: {neighbor['score']:.3f}")
        ...
        logger.info(f"✓ Found {len(citation_neighbors)} neighbors using citation edges only")
        ...
        logger.info(f"✓ Found {len(high_weight_neighbors)} neighbors with min_weight=0.5")
        ...
        if len(neighbors_2hop) >= 2:
            ...
            if abs(first_neighbor['score'] - expected_score) < 0.001:
                logger.info("✓ Score calculation is correct (50% path + 30% quality + 20% novelty)")
            else:
                logger.warning(f"⚠ Score mismatch: expected {expected_score:.3f}, got {first_neighbor['score']:.3f}")
        ...
        if len(limited_neighbors) <= 3:
            logger.info(f"✓ Limit parameter works correctly (requested 3, got {len(limited_neighbors)})")
        else:
            logger.warning(f"⚠ Limit parameter not working (requested 3, got {len(limited_neighbors)})")
        ...
        logger.info(f"✓ 1-hop neighbor discovery: PASSED ({len(neighbors_1hop)} neighbors)")
        logger.info(f"✓ 2-hop neighbor discovery: PASSED ({len(neighbors_2hop)} neighbors)")
        logger.info(f"✓ Edge type filtering: PASSED ({len(citation_neighbors)} citation neighbors)")
        logger.info(f"✓ Min weight filtering: PASSED ({len(high_weight_neighbors)} high-weight neighbors)")
