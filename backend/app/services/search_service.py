...
class AdvancedSearchService:
    ...
    def search_three_way_hybrid(
        ...
        Returns:
            ...
            - facets: Faceted search results
            - snippets: Dict mapping resource_id to snippet text
            - metadata: Dict with latency, method contributions, weights used
        """
        import time
        ...
        # Check if Phase 8 services are available
        if (SparseEmbeddingService is None or 
            ReciprocalRankFusionService is None or 
            RerankingService is None):
            logger.warning(
                "Phase 8 services not available, falling back to two-way hybrid search"
            )
            # Fall back to existing hybrid search
            results = AdvancedSearchService.hybrid_search(
                db, query, query.hybrid_weight or 0.5
            )
            # Unpack and add empty metadata
            if len(results) == 4:
                resources, total, facets, snippets = results
        ...
        metadata = {
            'latency_ms': 0.0,
            'method_contributions': {'fts5': 0, 'dense': 0, 'sparse': 0},
            'weights_used': [1.0/3, 1.0/3, 1.0/3],
            'reranking_enabled': enable_reranking,
            'adaptive_weighting': adaptive_weighting
        }
        
        # Analyze query for adaptive weighting
        query_analysis = AdvancedSearchService._analyze_query(query.text)
        
        # Execute three retrieval methods in parallel (simulated with sequential for now)
        # TODO: Implement true parallel execution with asyncio for better performance
        ...
        try:
            if query.text and _detect_fts5(db) and _fts_index_ready(db):
                ...
                fts5_results = [(str(item.id), bm25_scores.get(str(item.id), 1.0)) 
                               for item in items]
                metadata['method_contributions']['fts5'] = len(fts5_results)
        except Exception as e:
            logger.error(f"FTS5 search failed: {e}", exc_info=True)
                ...
                if has_embeddings:
                    ...
                    if query_embedding is not None:
                        ...
                        for resource in resources:
                            try:
                                import json
                                resource_embedding = json.loads(resource.embedding)
                                
                                # Compute cosine similarity
                                similarity = float(np.dot(query_embedding, resource_embedding) / 
                                                 (np.linalg.norm(query_embedding) * 
                                                  np.linalg.norm(resource_embedding)))
                                
                                scores.append((str(resource.id), similarity))
                            except Exception:
                                continue
                        
                        # Sort by similarity descending and take top 100
                        scores.sort(key=lambda x: x[1], reverse=True)
                        dense_results = scores[:100]
                        metadata['method_contributions']['dense'] = len(dense_results)
        except Exception as e:
            logger.error(f"Dense vector search failed: {e}", exc_info=True)
        ...
        try:
            if query.text:
                sparse_results = AdvancedSearchService._search_sparse(db, query.text, limit=100)
                metadata['method_contributions']['sparse'] = len(sparse_results)
        except Exception as e:
            logger.error(f"Sparse vector search failed: {e}", exc_info=True)
        
        # Check if we have any results
        if not fts5_results and not dense_results and not sparse_results:
            logger.warning("All three retrieval methods returned empty results")
        ...
        if adaptive_weighting:
            # Use query-adaptive weights
            weights = rrf_service.adaptive_weights(query.text, None)
        ...
        try:
            result_lists = [fts5_results, dense_results, sparse_results]
            merged_results = rrf_service.fuse_results(result_lists, weights)
        except Exception as e:
            logger.error(f"RRF fusion failed: {e}", exc_info=True)
            # Fall back to first non-empty result list
            merged_results = fts5_results or dense_results or sparse_results
        
        # Extract top candidates for reranking
        candidate_ids = [rid for rid, _ in merged_results[:100]]
        
        # Apply reranking if enabled
        if enable_reranking and candidate_ids:
            try:
                reranking_service = RerankingService(db)
                reranked_results = reranking_service.rerank(
                    query.text,
                ...
                if reranked_results:
                    ...
                    logger.debug(f"Reranking applied to {len(candidate_ids)} candidates")
            except Exception as e:
                logger.error(f"Reranking failed: {e}", exc_info=True)
                # Continue with RRF results
        
        # Apply pagination
        offset = query.offset or 0
        limit = query.limit or 20
        
        # Get paginated resource IDs
        paginated_ids = [rid for rid, _ in merged_results[offset:offset + limit]]
        
        # Fetch resources preserving order
        resources = AdvancedSearchService._fetch_resources_ordered(
            db, paginated_ids, query.filters
        )
        
        # Compute total count (from merged results before pagination)
        total = len(merged_results)
        
        # Compute facets from all merged results
        all_resource_ids = [rid for rid, _ in merged_results]
        facets_query = db.query(Resource).filter(Resource.id.in_(all_resource_ids))
        facets_query = _apply_structured_filters(facets_query, query.filters)
        facets = _compute_facets(db, facets_query)
        
        # Generate snippets for paginated results
        snippets = {}
        if query.text:
            for resource in resources:
                snippets[str(resource.id)] = AdvancedSearchService.generate_snippets(
        ...
        if metadata['latency_ms'] > 500:
            logger.warning(
                f"Slow three-way hybrid search query: {metadata['latency_ms']:.1f}ms "
                f"for query '{query.text[:50]}...'"
            )
        
        logger.info(
            f"Three-way hybrid search completed in {metadata['latency_ms']:.1f}ms: "
            f"FTS5={metadata['method_contributions']['fts5']}, "
            f"Dense={metadata['method_contributions']['dense']}, "
            f"Sparse={metadata['method_contributions']['sparse']}, "
            f"Total={total}, Returned={len(resources)}"
        )
