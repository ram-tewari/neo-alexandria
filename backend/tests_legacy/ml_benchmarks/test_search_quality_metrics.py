...
class TestSearchQualityMetrics:
    ...
    def test_hybrid_search_ndcg(
        self, isolated_test_db, search_relevance_data: Dict[str, Any]
    ):
        """
        ...
        for query_data in queries:
            ...
            try:
                ...
                search_results = AdvancedSearchService.search(
                    isolated_test_db, search_query
                )
                ...
                if sum(true_relevance) > 0:
                    # Compute NDCG@20
                    ndcg = ndcg_score([true_relevance], [pred_scores], k=20)
    ...
    def test_precision_recall_at_10(
        ...
        for query_data in queries:
            ...
            try:
                ...
                precision = (
                    len(relevant_retrieved) / len(top_10_ids) if top_10_ids else 0.0
                )
        ...
        if precision_scores:
            print(
                f"  Precision - Min: {np.min(precision_scores):.4f}, Max: {np.max(precision_scores):.4f}"
            )
            print(
                f"  Recall - Min: {np.min(recall_scores):.4f}, Max: {np.max(recall_scores):.4f}"
            )
    ...
    def test_component_comparison(
        ...
        print("-" * 80)
        print(
            f"{'Configuration':<20} {'Avg NDCG@20':>15} {'Queries':>10} {'Relative':>15}"
        )
        print("-" * 80)

        # Find best configuration
        best_config = (
            max(config_averages.items(), key=lambda x: x[1])
            if config_averages
            else (None, 0.0)
        )
        ...
        for config_name in ["FTS5-only", "Dense-only", "Hybrid (0.5)"]:
            ...
            print(
                f"{config_name:<20} {avg_ndcg:>15.4f} {num_queries:>10} {relative:>14.1f}%{marker}"
            )
