...
def test_annotation_creation_flow(db_session, create_test_resource, mock_event_bus):
    ...
    resource = create_test_resource(
        title="Test Document",
        description="A document for annotation testing"
    )
...
def test_annotation_update_flow(db_session, create_test_resource, create_test_annotation):
    """
    ...
    with pytest.raises(PermissionError, match="Cannot update another user's annotation"):
...
def test_annotation_delete_flow(db_session, create_test_resource, create_test_annotation):
    """
    ...
    with pytest.raises(PermissionError, match="Cannot delete another user's annotation"):
...
def test_annotation_access_control_flow(db_session, create_test_resource, create_test_annotation):
    """
    ...
    private_retrieved = service.get_annotation_by_id(
        annotation_id=str(private_ann.id),
        user_id="user2"
    )
    ...
    shared_retrieved = service.get_annotation_by_id(
        annotation_id=str(shared_ann.id),
        user_id="user2"
    )
...
def test_annotation_tag_filtering_flow(db_session, create_test_resource, create_test_annotation):
    """
