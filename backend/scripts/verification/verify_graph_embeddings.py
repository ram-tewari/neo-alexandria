...
def test_service_initialization():
    ...
    try:
        ...
        assert service._graph_service is None, (
            "GraphService should not be loaded initially"
        )
        ...
        assert isinstance(graph_service, GraphService), (
            "Should be GraphService instance"
        )
...
def test_graph2vec_computation():
    ...
    try:
        ...
        logger.info(
            f"Database state: {resource_count} resources, {edge_count} graph edges"
        )
        ...
        stats = service.compute_graph2vec_embeddings(
            dimensions=128, wl_iterations=2, batch_size=100
        )
        ...
        if sample_embedding:
            ...
            assert sample_embedding.embedding_method == "graph2vec", (
                "Method should be graph2vec"
            )
            assert sample_embedding.embedding_version == "v1.0", (
                "Version should be v1.0"
            )
            assert len(sample_embedding.structural_embedding) == 128, (
                "Should have 128 dimensions"
            )
        ...
        if stats["total_nodes"] > 0 and stats["processing_time_seconds"] > 0:
            nodes_per_minute = (
                stats["total_nodes"] / stats["processing_time_seconds"]
            ) * 60
