    ...
    try:
        while True:
            ...
            if error_rate > ERROR_RATE_THRESHOLD:
                logger.error(f"High error rate detected: {error_rate:.4f} > {ERROR_RATE_THRESHOLD:.4f}")
                logger.error("Triggering automatic rollback...")
                
                rollback_success = rollback_to_previous_version(
                    versioning,
                    reason=f"High error rate: {error_rate:.4f}"
                )
            ...
            if latency_p95 > LATENCY_P95_THRESHOLD_MS:
                logger.error(f"High latency detected: {latency_p95:.2f}ms > {LATENCY_P95_THRESHOLD_MS}ms")
                logger.error("Triggering automatic rollback...")
                
                rollback_success = rollback_to_previous_version(
                    versioning,
                    reason=f"High latency: {latency_p95:.2f}ms"
                )
            ...
            if max_duration_seconds is not None:
                ...
                if elapsed >= max_duration_seconds:
                    logger.info(f"Max duration reached ({max_duration_seconds}s), exiting monitor")
                    break
...
def main():
    ...
    monitor_parser.add_argument('--base-dir', default='models/classification',
                                help='Base directory for models')
    
    # Rollback command
    rollback_parser = subparsers.add_parser('rollback', help='Manually trigger rollback')
    rollback_parser.add_argument('--reason', default='Manual rollback',
                                 help='Reason for rollback')
    rollback_parser.add_argument('--base-dir', default='models/classification',
                                help='Base directory for models')
    
    # Check metrics command
    metrics_parser = subparsers.add_parser('metrics', help='Check current metrics')
    metrics_parser.add_argument('--base-dir', default='models/classification',
                               help='Base directory for models')
