...
class ABTestingFramework:
    ...
    def create_experiment(
        ...
        control_model = self.db.query(ModelVersion).filter(
            ModelVersion.version == control_version
        ).first()
        ...
        treatment_model = self.db.query(ModelVersion).filter(
            ModelVersion.version == treatment_version
        ).first()
    ...
    def route_prediction(
        ...
        experiment = self.db.query(ABTestExperiment).filter(
            ABTestExperiment.id == UUID(experiment_id)
        ).first()
    ...
    def _flush_prediction_buffer(self):
        ...
        try:
            ...
            prediction_logs = [
                ...
                for log_data in self.prediction_buffer
            ]
            ...
            logger.info(f"Flushed {len(self.prediction_buffer)} predictions to database")
    ...
    def analyze_experiment(
        ...
        control_logs = self.db.query(PredictionLog).filter(
        ...
        treatment_logs = self.db.query(PredictionLog).filter(
            PredictionLog.experiment_id == UUID(experiment_id),
        ...
        control_confidences = [
            max(log.predictions.values()) for log in control_logs
        ]
        ...
        results = {
            ...
            'statistical_significance': {
                ...
                'confidence_interval': [float(confidence_ci[0]), float(confidence_ci[1])]
            },
