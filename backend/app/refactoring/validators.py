...
class FunctionLengthChecker:
    ...
    def _analyze_function(self, node: ast.FunctionDef, source: str) -> FunctionInfo:
    ...
    def _create_smell(self, file_path: Path, func: FunctionInfo) -> CodeSmell:
        ...
        severity = (
            Severity.HIGH if func.line_count > self.max_lines * 2 else Severity.MEDIUM
        )
...
class ClassSizeChecker:
    ...
    def __init__(self, max_lines: int = MAX_CLASS_LINES, max_methods: int = 10):
        """
    ...
    def check_file(self, file_path: Path) -> List[CodeSmell]:
        ...
        try:
            ...
            for cls in classes:
                if (
                    cls.line_count > self.max_lines
                    or cls.public_method_count > self.max_methods
                ):
    ...
    def _create_smell(self, file_path: Path, cls: ClassInfo) -> CodeSmell:
        ...
        if cls.public_method_count > self.max_methods:
            reasons.append(
                f"{cls.public_method_count} public methods (max: {self.max_methods})"
            )

        severity = (
            Severity.HIGH if cls.line_count > self.max_lines * 1.5 else Severity.MEDIUM
        )
...
class TypeHintCoverageChecker:
    ...
    def _create_smell(self, file_path: Path, node: ast.FunctionDef) -> CodeSmell:
            ...
            for func1, body1 in bodies1:
                for file2, bodies2 in function_bodies.items():
                    for func2, body2 in bodies2:
                        ...
                        pair_key = tuple(
                            sorted([f"{file1}:{func1.name}", f"{file2}:{func2.name}"])
                        )
