...
class PredictionMonitor:
    ...
    def get_metrics(self, window_minutes: int = 60) -> Dict[str, Any]:
        ...
        low_confidence_rate = low_confidence_count / total_predictions if total_predictions > 0 else 0.0
        ...
        logger.debug(f"Calculated metrics for {total_predictions} predictions in {window_minutes}min window")
    ...
    def clear_old_predictions(self, retention_hours: int = 24) -> int:
        ...
        if removed_count > 0:
            logger.info(f"Cleared {removed_count} old predictions (retention: {retention_hours}h)")
