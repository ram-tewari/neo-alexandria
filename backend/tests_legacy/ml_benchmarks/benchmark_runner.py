...
class BenchmarkRunner:
    ...
    def _run_classification(self) -> BenchmarkSuiteResult:
        ...
        suite_result = self._parse_pytest_results(
            result_file, "classification", execution_time
        )
    ...
    def _run_collaborative_filtering(self) -> BenchmarkSuiteResult:
        ...
        # Parse results
        suite_result = self._parse_pytest_results(
            result_file, "collaborative_filtering", execution_time
        )
        ...
        print(
            f"✓ Collaborative filtering benchmarks completed in {execution_time:.2f}s"
        )
        print(f"  Passed: {suite_result.passed_tests}/{suite_result.total_tests}")
    ...
    def _run_search_quality(self) -> BenchmarkSuiteResult:
        ...
        # Parse results
        suite_result = self._parse_pytest_results(
            result_file, "search_quality", execution_time
        )
    ...
    def _run_summarization_quality(self) -> BenchmarkSuiteResult:
        ...
        # Parse results
        suite_result = self._parse_pytest_results(
            result_file, "summarization_quality", execution_time
        )
    ...
    def _run_performance(self) -> BenchmarkSuiteResult:
        ...
        # Parse results
        suite_result = self._parse_pytest_results(
            result_file, "performance", execution_time
        )
    ...
    def _parse_pytest_results(
        self, result_file: Path, suite_name: str, execution_time: float
        ...
        try:
            if not result_file.exists():
                ...
                return BenchmarkSuiteResult(
                    ...
                    recommendations=[
                        "Result file not found - tests may have been skipped"
                    ],
                )
            ...
            recommendations = self._generate_suite_recommendations(
                suite_name, results, failed_tests
            )
    ...
    def _generate_suite_recommendations(
        self, suite_name: str, results: List[BenchmarkResult], failed_count: int
    ) -> List[str]:
        """
        ...
        if suite_name == "classification":
            ...
            recommendations.append("Consider using a different model or fine-tuning")
        elif suite_name == "performance":
            recommendations.append("Profile code to identify performance bottlenecks")
            recommendations.append("Consider batch processing or caching optimizations")
    ...
    def _generate_basic_report(self) -> None:
        ...
        report_lines.append("")
        report_lines.append(
            "| Suite | Total Tests | Passed | Failed | Duration (s) | Status |"
        )
        report_lines.append(
            "|-------|-------------|--------|--------|--------------|--------|"
        )
        ...
        for suite_name, suite_result in self.results.items():
            ...
            report_lines.append("")
            report_lines.append(
                f"**Execution Time**: {suite_result.execution_time_seconds:.2f}s"
            )
            report_lines.append(
        ...
        if total_failed == 0:
            report_lines.append(
                "✅ All benchmarks passed! The ML system is performing well."
            )
        else:
        ...
        report_lines.append(
            "python backend/tests/ml_benchmarks/benchmark_runner.py --report-only"
        )
        report_lines.append("```")
        report_lines.append("")
    ...
    def _print_summary(self) -> None:
        ...
        print(
            f"Failed: {total_failed} ({100 * total_failed / total_tests if total_tests > 0 else 0:.1f}%)"
        )
