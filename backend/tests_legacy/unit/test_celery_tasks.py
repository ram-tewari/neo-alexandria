...
class TestRegenerateEmbeddingTask:
    ...
    def test_task_configuration(self):
        ...
        assert (
            regenerate_embedding_task.name
            == "app.tasks.celery_tasks.regenerate_embedding_task"
        )
...
class TestRecomputeQualityTask:
    ...
    def test_successful_quality_recomputation(
        self, mock_quality_service, mock_base, mock_session_local
    ):
    ...
    @patch("app.tasks.celery_tasks.Base")
    @patch("app.services.quality_service.QualityService")
    def test_quality_task_error_handling(
        self, mock_quality_service, mock_base, mock_session_local
    ):
        ...
        mock_service_instance = MagicMock()
        mock_quality_service.return_value = mock_service_instance
        mock_service_instance.compute_quality.side_effect = Exception(
...
class TestUpdateSearchIndexTask:
    ...
    def test_task_configuration(self):
        ...
        assert (
            update_search_index_task.name
            == "app.tasks.celery_tasks.update_search_index_task"
        )
...
class TestUpdateGraphEdgesTask:
    ...
    def test_successful_graph_edge_update(
        ...
        mock_service_instance.add_citation_edges.assert_called_once_with(
            "resource-123", citations
        )
...
class TestInvalidateCacheTask:
    ...
    def test_task_configuration(self):
        ...
        assert (
            invalidate_cache_task.name == "app.tasks.celery_tasks.invalidate_cache_task"
        )
...
class TestRefreshRecommendationProfileTask:
    ...
    def test_task_configuration(self):
        """Test task is properly configured."""
        assert (
            refresh_recommendation_profile_task.name
...
class TestScheduledMaintenanceTasks:
    ...
    def test_cleanup_expired_cache_configuration(self):
        ...
        assert (
            cleanup_expired_cache_task.name
            == "app.tasks.celery_tasks.cleanup_expired_cache_task"
        )
