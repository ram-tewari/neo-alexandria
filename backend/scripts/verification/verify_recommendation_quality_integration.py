...
def test_quality_boosting():
    ...
    candidates = [
        ...
        (
            np.array([0.2] * 384),
            {"url": "https://example.com/new", "title": "New Resource", "snippet": ""},
        ),
    ]
    ...
    results_with_boost = score_candidates(
        profile, candidates, ["test"], 10, db=db, quality_boost_factor=1.2
    )
...
def test_recommendations_filter_outliers():
    ...
    try:
        ...
        print(
            f"   ✓ Function accepts quality parameters (failed gracefully: {type(e).__name__})"
        )
...
def test_api_parameters():
    ...
    assert "quality_boost_factor" in params, (
        "API should accept quality_boost_factor parameter"
    )
