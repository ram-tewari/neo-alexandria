...
class TestVersionCreation:
    ...
    def test_create_version_with_valid_format(self, versioning, mock_model_dir):
        ...
        version_path = versioning.create_version(
            model_path=str(mock_model_dir), version=version, metadata=metadata
        )
        ...
        for invalid_version in invalid_versions:
            with pytest.raises(ValueError, match="Invalid version format"):
                versioning.create_version(
                    model_path=str(mock_model_dir), version=invalid_version, metadata={}
                )

    def test_create_version_rejects_duplicate_versions(
        self, versioning, mock_model_dir
    ):
    ...
    def test_create_version_copies_model_files(self, versioning, mock_model_dir):
        ...
        version_path = versioning.create_version(
            model_path=str(mock_model_dir), version=version, metadata={}
        )
...
class TestVersionLoading:
    ...
    def test_load_version_returns_model_and_metadata(self, versioning, mock_model_dir):
...
class TestProductionPromotion:
    ...
    def test_promote_to_production_updates_registry(self, versioning, mock_model_dir):
        ...
        version_entry = next(
            v for v in versioning.registry["versions"] if v["version"] == version
        )
    ...
    def test_promote_to_production_archives_previous(self, versioning, mock_model_dir):
        ...
        v1_entry = next(
            v for v in versioning.registry["versions"] if v["version"] == v1
        )
...
class TestVersionComparison:
    ...
    def test_compare_versions_returns_metrics(self, versioning, mock_model_dir):
