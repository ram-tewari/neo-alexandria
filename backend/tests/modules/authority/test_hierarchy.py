...
class TestAuthorityHierarchy:
    ...
    def test_authority_hierarchy_validation(self, db_session):
        ...
        **Validates: Requirements 10.1, 10.3**
        ...
        expected_codes = ["000", "100", "200", "300", "400", "500", "600", "700", "800", "900"]
        ...
        for code in all_codes:
            if code.parent_code is not None:
                ...
                parent = db_session.query(ClassificationCode).filter_by(code=code.parent_code).first()
                assert parent is not None, f"Parent {code.parent_code} not found for code {code.code}"
                
                # Verify no self-reference
                assert code.code != code.parent_code, f"Code {code.code} references itself as parent"
    ...
    def test_authority_assignment_flow(self, db_session, mock_event_bus):
        ...
        **Validates: Requirements 10.1, 10.3, 10.2-10.6, 13.1-13.3**
        ...
        assert "Artificial Intelligence" in normalized_subjects  # "ai" -> "Artificial Intelligence"
        ...
        from app.database.models import AuthoritySubject, AuthorityCreator, AuthorityPublisher
        
        # Check subjects
        for subject in normalized_subjects:
            subject_record = db_session.query(AuthoritySubject).filter_by(canonical_form=subject).first()
            assert subject_record is not None
            assert subject_record.usage_count > 0
        
        # Check creator
        creator_record = db_session.query(AuthorityCreator).filter_by(canonical_form=normalized_creator).first()
        assert creator_record is not None
        assert creator_record.usage_count > 0
        
        # Check publisher
        publisher_record = db_session.query(AuthorityPublisher).filter_by(canonical_form=normalized_publisher).first()
