    ...
    try:
        ...
        if check_size:
            ...
            if size_check["passed"]:
                logger.info(f"  ✓ Model size acceptable: {size_check['size_mb']:.2f} MB")
            else:
        ...
        if run_smoke_tests:
            ...
            validation_results["checks"]["smoke_tests_passed"] = smoke_test_results["passed"]
            validation_results["checks"]["smoke_test_details"] = smoke_test_results["details"]
            
            if smoke_test_results["passed"]:
                logger.info(f"  ✓ Smoke tests passed ({smoke_test_results['num_passed']}/{smoke_test_results['num_tests']})")
            else:
                ...
                for test in smoke_test_results["details"]:
                    if not test["passed"]:
                        logger.error(f"    Failed: {test['input'][:50]}... - {test['error']}")
        ...
        if check_latency:
            ...
            validation_results["checks"]["latency_p95_ms"] = latency_check["latency_p95"]
            
            if latency_check["passed"]:
                logger.info(f"  ✓ Latency acceptable: {latency_check['latency_p95']:.2f} ms (p95)")
            else:
        ...
        if quality_check["passed"]:
            ...
            logger.info(f"    Valid predictions: {quality_check['num_valid']}/{quality_check['num_tests']}")
        else:
        ...
        validation_results["passed"] = (
            validation_results["checks"].get("model_loaded", False) and
            validation_results["checks"].get("size_acceptable", True) and
            validation_results["checks"].get("smoke_tests_passed", True) and
            validation_results["checks"].get("predictions_reasonable", True)
...
def check_model_size(
    ...
    return {
        "passed": passed,
        "size_mb": size_mb,
        "threshold_mb": MAX_MODEL_SIZE_MB
    }
...
def main():
    ...
    # Validate command
    validate_parser = subparsers.add_parser('validate', help='Validate model deployment')
    validate_parser.add_argument('--version', required=True, help='Model version to validate')
    validate_parser.add_argument('--base-dir', default='models/classification',
                                 help='Base directory for models')
    validate_parser.add_argument('--no-smoke-tests', action='store_true',
                                 help='Skip smoke tests')
    validate_parser.add_argument('--no-size-check', action='store_true',
                                 help='Skip size check')
    validate_parser.add_argument('--no-latency', action='store_true',
                                 help='Skip latency check')
    validate_parser.add_argument('--output', default='validation_results.json',
