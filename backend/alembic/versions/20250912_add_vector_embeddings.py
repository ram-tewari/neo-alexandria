...
def upgrade() -> None:
    ...
    if bind.dialect.name == 'postgresql':
        ...
        op.add_column('resources', sa.Column('embedding', postgresql.ARRAY(sa.Float), nullable=True, server_default='{}'))
    else:
        # SQLite and others: Use JSON column for cross-database compatibility
        op.add_column('resources', sa.Column('embedding', sa.JSON, nullable=True, server_default='[]'))
