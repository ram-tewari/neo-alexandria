...
class EndToEndPipeline:
    ...
    def step1_collect_data(self, num_papers: int = 10000) -> Path:
        ...
        def collect():
            ...
            collector.create_classification_dataset(
                num_papers=num_papers, output_file=str(output_file)
            )
            ...
            self.results["metrics"]["categories"] = data.get("metadata", {}).get(
                "num_categories", 0
            )

            logger.info(
                f"Collected {self.results['metrics']['collected_papers']} papers"
            )
            return output_file
    ...
    def step2_preprocess_data(self, input_file: Path) -> Path:
        ...
        def preprocess():
            ...
            output_file = (
                self.output_dir / "processed" / "arxiv_classification_clean.json"
            )
            output_file.parent.mkdir(parents=True, exist_ok=True)

            logger.info("Preprocessing dataset...")
            result = preprocessor.preprocess_dataset(
                input_file=str(input_file), output_file=str(output_file), min_words=50
            )

            self.results["metrics"]["preprocessed_samples"] = result.get(
                "num_samples", 0
            )
            self.results["metrics"]["removed_samples"] = result.get(
                "removed_samples", 0
            )

            logger.info(
                f"Preprocessed {self.results['metrics']['preprocessed_samples']} samples"
            )
            return output_file
    ...
    def step4_train_model(self, splits_dir: Path) -> Path:
        ...
        def train():
            ...
            trainer = ClassificationTrainer(
                model_name="distilbert-base-uncased", output_dir=str(model_dir)
            )
            ...
            self.results["metrics"]["train_accuracy"] = train_result.get(
                "train_accuracy", 0
            )
            self.results["metrics"]["val_accuracy"] = train_result.get(
                "val_accuracy", 0
            )

            logger.info(
                f"Training complete: val_accuracy={self.results['metrics']['val_accuracy']:.4f}"
            )
            return model_dir
    ...
    def step5_evaluate_model(self, model_dir: Path, splits_dir: Path) -> Dict[str, Any]:
        ...
        def evaluate():
            ...
            logger.info(
                f"Evaluation complete: test_accuracy={self.results['metrics']['test_accuracy']:.4f}"
            )
            return eval_result
    ...
    def step6_create_version(self, model_dir: Path) -> str:
        ...
        def create_version():
            ...
            metadata = {
                ...
                "dataset": {
                    "source": "arXiv",
                    "num_samples": self.results["metrics"].get(
            ...
            versioning.create_version(
                model_path=str(model_dir), version=version, metadata=metadata
            )
    ...
    def step7_validate_deployment(self, version: str) -> Dict[str, Any]:
        ...
        def validate():
            ...
            self.results["metrics"]["model_size_mb"] = validation_result.get(
                "model_size_mb", 0
            )
            self.results["metrics"]["load_time_s"] = validation_result.get(
                "load_time_s", 0
            )
            self.results["metrics"]["inference_latency_ms"] = validation_result.get(
                "inference_latency_ms", 0
            )

            logger.info(
                f"Validation complete: model_size={self.results['metrics']['model_size_mb']:.2f}MB"
            )
    ...
    def print_summary(self):
        ...
        print("\nData Collection:")
        print(
            f"  - Papers collected: {self.results['metrics'].get('collected_papers', 0)}"
        )
        ...
        print("\nPreprocessing:")
        print(
            f"  - Samples after cleaning: {self.results['metrics'].get('preprocessed_samples', 0)}"
        )
        print(
            f"  - Samples removed: {self.results['metrics'].get('removed_samples', 0)}"
        )
        ...
        print("\nTraining:")
        print(
            f"  - Train accuracy: {self.results['metrics'].get('train_accuracy', 0):.4f}"
        )
        print(f"  - Val accuracy: {self.results['metrics'].get('val_accuracy', 0):.4f}")
        print(
            f"  - Time: {self.results['timings'].get('4_train_model', 0) / 3600:.2f} hours"
        )

        print("\nEvaluation:")
        print(
            f"  - Test accuracy: {self.results['metrics'].get('test_accuracy', 0):.4f}"
        )
        ...
        print(
            f"  - Inference latency: {self.results['metrics'].get('inference_latency_ms', 0):.2f}ms"
        )
