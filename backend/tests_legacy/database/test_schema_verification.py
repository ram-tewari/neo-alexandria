...
class TestDatabaseSchemaVerification:
    ...
    def test_resource_creation_with_all_critical_fields(self, test_db):
        ...
        try:
            ...
            assert (
                ...
                == '{"token_1": 0.9, "token_2": 0.7, "token_3": 0.5}'
            )
            ...
            queried_resource = (
                db.query(Resource).filter(Resource.id == resource.id).first()
            )
    ...
    def test_multiple_resources_with_sparse_embeddings(self, test_db):
        ...
        try:
            ...
            queried = (
                db.query(Resource).filter(Resource.sparse_embedding.isnot(None)).all()
            )
