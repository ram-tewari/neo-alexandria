...
def test_resource_deletion_emits_event(db_session: Session):
    ...
    def capture_event(event_name, payload, priority=None):
        emitted_events.append(
            {"event": event_name, "payload": payload, "priority": priority}
        )
    ...
    resource_deleted_events = [
        e for e in emitted_events if e["event"] == SystemEvent.RESOURCE_DELETED.value
    ]

    assert len(resource_deleted_events) == 1, (
        "resource.deleted event should be emitted once"
    )
...
def test_collections_handler_receives_event(db_session: Session):
    ...
    payload = {"resource_id": resource_id, "title": "Test Handler Resource"}
    ...
    assert resource_id not in resource_ids, (
        "Deleted resource should not be in collection"
    )
...
def test_no_circular_imports():
    ...
    for line in lines:
        ...
        if "CollectionService" in line:
            assert False, (
                f"Found CollectionService reference in ResourceService: {line.strip()}"
            )
...
def test_end_to_end_event_flow(db_session: Session):
    ...
    collection_service.add_resources_to_collection(
        collection.id, [resource1.id, resource2.id], "test_user"
    )
