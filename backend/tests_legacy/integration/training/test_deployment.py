...
def setup_test_versions(temp_model_dir, mock_model_data):
    ...
    # Create versions
    with patch.object(
        versioning, "load_version", return_value=(mock_model_data, metadata_v1)
    ):
...
class TestBlueGreenDeployment:
    ...
    def test_blue_green_deployment_initialization(
        self, temp_model_dir, setup_test_versions
    ):
        """Test blue-green deployment initializes correctly."""
        deployment = BlueGreenDeployment(
            blue_version="v1.0.0", green_version="v1.1.0", base_dir=temp_model_dir
        )
    ...
    def test_deploy_green_success(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
        ...
        with patch.object(
            deployment.versioning, "load_version", return_value=(mock_model_data, {})
        ):
...
class TestCanaryDeployment:
    ...
    def test_canary_deployment_initialization(
        self, temp_model_dir, setup_test_versions
    ):
    ...
    def test_gradual_rollout_success(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
    ...
    def test_gradual_rollout_rollback_on_degradation(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
    ...
    def test_metrics_acceptable(
        ...
        def mock_get_metrics(version_type):
            return {"error_rate": 0.0, "latency_p95": 50.0, "num_samples": 10}
...
class TestAutomaticRollback:
    ...
    def test_get_production_metrics(
        ...
        # Mock load_version to return mock model
        with patch.object(
            versioning, "load_version", return_value=(mock_model_data, {})
        ):
    ...
    def test_monitor_and_rollback_with_high_error_rate(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
        ...
        with patch(
            "backend.scripts.deployment.rollback_monitor.get_production_metrics",
            side_effect=mock_get_metrics,
        ):
            with patch(
                "backend.scripts.deployment.rollback_monitor.rollback_to_previous_version",
                return_value=True,
            ) as mock_rollback:
                with patch(
                    "backend.scripts.deployment.rollback_monitor.send_alert_notification"
                ):
...
class TestDeploymentValidation:
    ...
    def test_validate_deployment_success(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
        ...
        with patch.object(
            versioning,
            "load_version",
            return_value=(mock_model_data, {"model_name": "test"}),
        ):
            with patch(
                "backend.scripts.deployment.validate.ModelVersioning",
                return_value=versioning,
            ):
    ...
    def test_validate_deployment_model_load_failure(
        self, temp_model_dir, setup_test_versions
    ):
        ...
        with patch.object(
            versioning, "load_version", side_effect=Exception("Model not found")
        ):
            with patch(
                "backend.scripts.deployment.validate.ModelVersioning",
                return_value=versioning,
                ...
                results = validate_deployment(
                    version="v99.99.99", base_dir=temp_model_dir
                )
    ...
    def test_validate_deployment_size_check(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
        ...
        with patch.object(
            versioning, "load_version", return_value=(mock_model_data, metadata)
        ):
            with patch(
                "backend.scripts.deployment.validate.ModelVersioning",
                return_value=versioning,
    ...
    def test_validate_deployment_smoke_tests(
        self, temp_model_dir, setup_test_versions, mock_model_data
    ):
