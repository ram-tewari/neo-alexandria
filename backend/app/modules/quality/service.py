...
class QualityService:
    ...
    def get_quality_scores(self, resource_id: str, weights: Optional[Dict[str, float]] = None) -> Optional[QualityScore]:
    ...
    def compute_quality(self, resource_id: str, weights: Optional[Dict[str, float]] = None) -> QualityScore:
        ...
        self._update_resource_quality_fields(
            resource, accuracy, completeness, consistency, timeliness, relevance, overall, weights
        )
    ...
    def monitor_quality_degradation(self, time_window_days: int = DEGRADATION_DEFAULT_WINDOW_DAYS) -> List[Dict[str, Any]]:
    ...
    def detect_quality_outliers(self, batch_size: int = OUTLIER_DEFAULT_BATCH_SIZE) -> int:
        ...
        if len(resources) < OUTLIER_MIN_RESOURCES:
            raise ValueError(f"Outlier detection requires minimum {OUTLIER_MIN_RESOURCES} resources with quality scores")
        ...
        for resource in resources:
            ...
            features.append(resource.quality_accuracy if resource.quality_accuracy is not None else FEATURE_DEFAULT_VALUE)
            features.append(resource.quality_completeness if resource.quality_completeness is not None else FEATURE_DEFAULT_VALUE)
            features.append(resource.quality_consistency if resource.quality_consistency is not None else FEATURE_DEFAULT_VALUE)
            features.append(resource.quality_timeliness if resource.quality_timeliness is not None else FEATURE_DEFAULT_VALUE)
            features.append(resource.quality_relevance if resource.quality_relevance is not None else FEATURE_DEFAULT_VALUE)
    ...
    def _identify_outlier_reasons(self, resource) -> List[str]:
        ...
        if hasattr(resource, 'quality_accuracy') and resource.quality_accuracy and resource.quality_accuracy < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_accuracy")
        if hasattr(resource, 'quality_completeness') and resource.quality_completeness and resource.quality_completeness < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_completeness")
        if hasattr(resource, 'quality_consistency') and resource.quality_consistency and resource.quality_consistency < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_consistency")
        if hasattr(resource, 'quality_timeliness') and resource.quality_timeliness and resource.quality_timeliness < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_timeliness")
        if hasattr(resource, 'quality_relevance') and resource.quality_relevance and resource.quality_relevance < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_relevance")
        
        # Check summary quality dimensions
        if hasattr(resource, 'summary_coherence') and resource.summary_coherence and resource.summary_coherence < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_summary_coherence")
        if hasattr(resource, 'summary_consistency') and resource.summary_consistency and resource.summary_consistency < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_summary_consistency")
        if hasattr(resource, 'summary_fluency') and resource.summary_fluency and resource.summary_fluency < OUTLIER_THRESHOLD_LOW:
            reasons.append("low_summary_fluency")
        if hasattr(resource, 'summary_relevance') and resource.summary_relevance and resource.summary_relevance < OUTLIER_THRESHOLD_LOW:
