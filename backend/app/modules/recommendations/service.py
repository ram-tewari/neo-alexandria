...
def get_graph_based_recommendations(
    db: Session, resource_id: UUID, limit: int = 10
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        recommendations = strategy.generate(user_id=str(resource_id), limit=limit)
...
def generate_recommendations_with_graph_fusion(
    db: Session, resource_id: UUID, limit: int = 10, graph_weight: float = 0.3
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        recommendations = hybrid_strategy.generate(
            user_id=str(resource_id), limit=limit
        )
...
def generate_recommendations(
    ...
    try:
        ...
        recommendations = recommendation_strategy.generate(
            user_id=effective_user_id, limit=limit
        )
...
def generate_user_profile_vector(db: Session, user_id: str) -> List[float]:
    ...
    """
    try:
        from .strategies import ContentBasedStrategy
...
def recommend_based_on_annotations(
    ...
    try:
        ...
        recommendations = strategy.generate(user_id=user_id, limit=limit)
...
def get_top_subjects(db: Session, limit: int = 10) -> List[str]:
    ...
    try:
        ...
        for resource in resources:
            if resource.subject:
                subjects = (
                    resource.subject if isinstance(resource.subject, list) else []
                )
...
def _compute_gini_coefficient(values: List[float]) -> float:
    ...
    gini = (2.0 * np.sum((np.arange(1, n + 1) * sorted_values))) / (n * total) - (
        n + 1
    ) / n
...
def apply_novelty_boost(
    recommendations: List[Dict[str, Any]], boost_weight: float = 0.3
) -> List[Dict[str, Any]]:
    """
    ...
    for i, rec in enumerate(recommendations):
        ...
        position_novelty = (
            i / max(1, len(recommendations) - 1) if len(recommendations) > 1 else 0.0
        )
        ...
        adjusted_score = (
            original_score * (1.0 - boost_weight) + position_novelty * boost_weight
        )
