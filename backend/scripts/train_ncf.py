...
spec = importlib.util.spec_from_file_location("ncf_model", backend_dir / "app" / "models" / "ncf_model.py")
...
def create_negative_samples(
    ...
    logger.info(f"Total samples: {len(samples)} (positive: {len(positive_pairs)}, negative: {negative_count})")
    logger.info(f"Positive:Negative ratio: 1:{negative_count / len(positive_pairs):.2f}")
...
def evaluate_model(
    ...
    with torch.no_grad():
        for user_id, relevant_items in user_items.items():
            ...
            predictions = model.predict(user_tensor, item_tensor).cpu().numpy().flatten()
