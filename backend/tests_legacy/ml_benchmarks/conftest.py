...
def check_model_availability() -> Dict[str, bool]:
    ...
    return {"classification": classification_path.exists(), "ncf": ncf_path.exists()}
...
def trained_classifier(isolated_test_db: Session):
    ...
    """
    try:
        from backend.app.services.ml_classification_service import (
    ...
    try:
        ...
        service = MLClassificationService(
            isolated_test_db, model_version="benchmark_v1"
        )
