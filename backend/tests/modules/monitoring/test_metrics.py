...
def test_performance_metrics_collection(client: TestClient, mock_event_bus: MagicMock):
    ...
    with patch(
        "app.modules.monitoring.service.MonitoringService.get_performance_metrics"
    ) as mock_metrics:
        ...
        cache_hit_rate = (
            input_data["cache_hits"] / input_data["total_requests"]
            if input_data["total_requests"] > 0
            else 0.0
        )
        ...
        assert (
            actual_data["metrics"]["total_requests"] == expected_data["total_requests"]
        ), (
        ...
        assert (
            actual_data["metrics"]["slow_query_count"]
            == expected_data["slow_query_count"]
        ), (
...
def test_missing_metrics_handling(client: TestClient):
    ...
    with patch(
        ...
        assert (
            actual_data["metrics"]["cache_hit_rate"] == expected_data["cache_hit_rate"]
        ), (
...
def test_event_bus_metrics_collection(client: TestClient, mock_event_bus: MagicMock):
    ...
    with patch(
        "app.modules.monitoring.service.MonitoringService.get_event_bus_metrics"
    ) as mock_metrics:
        input_data = test_case["input"]
        expected_data = test_case["expected"]
        ...
        delivery_rate = (
            input_data["events_delivered"] / input_data["events_emitted"]
            if input_data["events_emitted"] > 0
            else 0.0
        )
        ...
        assert (
            actual_data["metrics"]["events_emitted"] == expected_data["events_emitted"]
        ), (
        ...
        assert (
            actual_data["metrics"]["events_delivered"]
            == expected_data["events_delivered"]
        ), (
