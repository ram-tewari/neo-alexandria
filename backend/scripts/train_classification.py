...
def load_classification_data(data_path: str = None) -> List[Tuple[str, List[str]]]:
    ...
    try:
        ...
        logger.info(f"  Average labels per sample: {sum(len(t) for _, t in labeled_data) / len(labeled_data):.2f}")
...
def augment_dataset(
    ...
    if original_size >= target_size:
        logger.info(f"Dataset size ({original_size}) already meets target ({target_size})")
...
def verify_checkpoint(checkpoint_dir: Path) -> bool:
    ...
    if (checkpoint_dir / "pytorch_model.bin").exists():
        ...
        logger.error("  ✗ Model weights - MISSING (neither pytorch_model.bin nor model.safetensors found)")
        return False
...
def main():
    ...
    try:
        ...
        service_path = Path(__file__).parent.parent / "app" / "services" / "ml_classification_service.py"
        spec = importlib.util.spec_from_file_location("ml_classification_service", service_path)
        ...
        if verify_checkpoint(checkpoint_dir):
            ...
            logger.info("  pytest backend/tests/ml_benchmarks/test_classification_metrics.py")
        else:
