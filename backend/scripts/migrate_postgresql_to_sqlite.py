#!/usr/bin/env python3
"""
Neo Alexandria 2.0 - PostgreSQL to SQLite Reverse Migration Script

This script migrates data from PostgreSQL back to SQLite for rollback scenarios.
It handles PostgreSQL-specific data types (JSONB, arrays) and converts them to
SQLite-compatible formats.

WARNING: This is a rollback/contingency script. Some PostgreSQL-specific features
will be lost during conversion:
- JSONB columns are converted to JSON text
- PostgreSQL full-text search vectors are not migrated
- Some PostgreSQL-specific indexes cannot be recreated in SQLite
- Array types are converted to JSON arrays

Usage:
    python migrate_postgresql_to_sqlite.py \\
        --source postgresql://user:pass@localhost:5432/neo_alexandria \\
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"reverse_migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
class PostgreSQLToSQLiteMigrator:
    """
    Handles reverse data migration from PostgreSQL to SQLite with data type conversion.

    This class manages the complete reverse migration process including:
    - Schema validation
    - Table dependency ordering
    - PostgreSQL-specific data type conversion (JSONB, arrays, etc.)
    - Batch data transfer
    - Progress reporting
    - Data validation
    ...
    - PostgreSQL full-text search vectors (tsvector) are not migrated
    - GIN indexes on JSONB columns cannot be recreated in SQLite
    - Some PostgreSQL-specific constraints may be lost
    - Array types are converted to JSON arrays
    """

    # Table migration order respecting foreign key dependencies
    # Same order as forward migration
    TABLE_ORDER = [
    ...
    # PostgreSQL-specific columns to skip during migration
    POSTGRESQL_ONLY_COLUMNS = {
        "resources": ["search_vector"],  # tsvector column for full-text search
    }
    ...
    def __init__(
        ...
        Args:
            source_url: PostgreSQL database connection URL
            target_url: SQLite database connection URL
        ...
        logger.warning(
            "This is a REVERSE migration - some PostgreSQL features will be lost"
        )
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            logger.info("Connecting to target database (SQLite)...")
    ...
    def validate_schemas(self) -> bool:
        ...
        logger.info(f"Source database (PostgreSQL) tables: {len(source_tables)}")
        logger.info(f"Target database (SQLite) tables: {len(target_tables)}")
        ...
        if missing_in_target:
            ...
            logger.error(
                "Please run Alembic migrations on target SQLite database first"
            )
            return False
        ...
        for table_name in self.TABLE_ORDER:
            if table_name not in source_tables:
                logger.warning(
                    f"Table '{table_name}' not found in source database (skipping)"
                )
                continue
            ...
            source_columns = {
                col["name"]: col for col in source_inspector.get_columns(table_name)
            }
            target_columns = {
                col["name"]: col for col in target_inspector.get_columns(table_name)
            }

            # Filter out PostgreSQL-only columns
            skip_columns = self.POSTGRESQL_ONLY_COLUMNS.get(table_name, [])
            for col in skip_columns:
                if col in source_columns:
                    logger.info(
                        f"Table '{table_name}': skipping PostgreSQL-only column '{col}'"
                    )
                    del source_columns[col]

            # Check for missing columns
            missing_columns = set(source_columns.keys()) - set(target_columns.keys())
            if missing_columns:
                logger.warning(
                    f"Table '{table_name}': columns in source but not in target: {missing_columns}"
                )
    ...
    def convert_postgresql_value(self, value: Any, column_type: str) -> Any:
        """
        Convert PostgreSQL-specific data types to SQLite-compatible formats.

        Args:
            value: The value to convert
            column_type: The PostgreSQL column type
        ...
        if "jsonb" in column_type.lower() or "json" in column_type.lower():
            if isinstance(value, (dict, list)):
                return json.dumps(value)
            return value

        # Convert PostgreSQL arrays to JSON arrays
        if "array" in column_type.lower() or isinstance(value, list):
            return json.dumps(value)
    ...
    def convert_row_data(
        self, row_dict: Dict[str, Any], table_name: str, source_columns: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Convert a row of data from PostgreSQL to SQLite-compatible format.

        Args:
            row_dict: Dictionary of column names to values
            table_name: Name of the table
            source_columns: Dictionary of column metadata from source database
        ...
        for col_name, value in row_dict.items():
            # Skip PostgreSQL-only columns
            if col_name in skip_columns:
                continue

            # Get column type
            col_info = source_columns.get(col_name, {})
            col_type = str(col_info.get("type", ""))

            # Convert value
            converted[col_name] = self.convert_postgresql_value(value, col_type)
    ...
    def migrate_table(self, table_name: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting reverse migration of table: {table_name}")
        ...
        try:
            ...
            if self.dry_run:
                logger.info(
                    f"Table '{table_name}': dry run mode, skipping data migration"
                )
                stats["success"] = True
                stats["end_time"] = datetime.now()
                return stats
            ...
            source_metadata.reflect(bind=self.source_engine, only=[table_name])
            source_table = source_metadata.tables[table_name]

            # Get target table metadata
            target_metadata = MetaData()
            target_metadata.reflect(bind=self.target_engine, only=[table_name])
            ...
            logger.info(f"Table '{table_name}': clearing existing data in target")
            self.target_session.execute(target_table.delete())
            ...
            while offset < stats["source_count"]:
                try:
                    ...
                    batch_query = (
                        select(source_table).limit(self.batch_size).offset(offset)
                    )
                    ...
                    for row in batch_rows:
                        ...
                        converted_row = self.convert_row_data(
                            row_dict, table_name, source_columns
                        )
                        batch_data.append(converted_row)

                    # Insert batch into target
                    if batch_data:
                        self.target_session.execute(target_table.insert(), batch_data)
                        self.target_session.commit()
                    ...
                    logger.info(
                        f"Table '{table_name}': migrated {migrated}/{stats['source_count']} rows ({progress_pct:.1f}%)"
                    )

                except SQLAlchemyError as e:
                    logger.error(
                        f"Error migrating batch at offset {offset} for table '{table_name}': {e}"
                    )
                    self.target_session.rollback()
                    stats["errors"].append({"offset": offset, "error": str(e)})
            ...
            if stats["success"]:
                logger.info(
                    f"Table '{table_name}': reverse migration completed successfully"
                )
            else:
                logger.warning(
                    f"Table '{table_name}': row count mismatch (source: {stats['source_count']}, target: {stats['target_count']})"
                )

        except Exception as e:
            logger.error(f"Fatal error migrating table '{table_name}': {e}")
            stats["errors"].append({"type": "fatal", "error": str(e)})
            stats["success"] = False
    ...
    def migrate_tables(self) -> None:
        ...
        logger.info("Starting reverse table migration...")
        ...
        for table_name in self.TABLE_ORDER:
            ...
            if table_stats["errors"]:
                self.errors.extend(
                    [{"table": table_name, **error} for error in table_stats["errors"]]
                )

        logger.info("Reverse table migration completed")
    ...
    def validate_data(self) -> bool:
        ...
        for table_name, stats in self.stats.items():
            if stats["source_count"] != stats["target_count"]:
                ...
                logger.debug(
                    f"Table '{table_name}': row counts match ({stats['source_count']} rows)"
                )
    ...
    def generate_report(self) -> str:
        ...
        report_lines = [
            "=" * 80,
            "REVERSE DATABASE MIGRATION REPORT (PostgreSQL → SQLite)",
            "=" * 80,
            "",
            f"Migration Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"Source Database (PostgreSQL): {self.source_url}",
            f"Target Database (SQLite): {self.target_url}",
        ...
        if self.warnings:
            report_lines.extend(
                [
                    "WARNINGS - Data Conversion Limitations",
                    "-" * 80,
                ]
            )
            ...
            report_lines.extend(
                [
                    "",
                    "Note: The following PostgreSQL features are not preserved:",
                    "- JSONB binary format (converted to JSON text)",
                    "- Full-text search vectors (tsvector columns)",
                    "- GIN indexes on JSONB columns",
                    "- PostgreSQL-specific array types (converted to JSON)",
                    "",
                ]
            )
    ...
    def migrate(self) -> bool:
        """
        Execute the complete reverse migration process.
        ...
        try:
            ...
            logger.error(f"Reverse migration failed with error: {e}")
            success = False
        finally:
            ...
            # Save report to file
            report_filename = f"reverse_migration_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            with open(report_filename, "w") as f:
                f.write(report)
            logger.info(f"Reverse migration report saved to: {report_filename}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate data from PostgreSQL back to SQLite (ROLLBACK)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
WARNING: This is a ROLLBACK script for contingency scenarios.
Some PostgreSQL-specific features will be lost during conversion:
- JSONB columns are converted to JSON text
- Full-text search vectors are not migrated
- GIN indexes cannot be recreated in SQLite
- Array types are converted to JSON arrays

Examples:
  # Basic reverse migration
  python migrate_postgresql_to_sqlite.py \\
      --source postgresql://user:pass@localhost:5432/neo_alexandria \\
      --target sqlite:///backend.db

  # Reverse migration with validation
  python migrate_postgresql_to_sqlite.py \\
      --source postgresql://user:pass@localhost:5432/neo_alexandria \\
      --target sqlite:///backend.db \\
      --validate
  ...
  python migrate_postgresql_to_sqlite.py \\
    ...
    parser.add_argument(
        "--source", required=True, help="Source database URL (PostgreSQL)"
    )
    parser.add_argument("--target", required=True, help="Target database URL (SQLite)")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=1000,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Validate schemas without migrating data"
    )
    ...
    # Verify source is PostgreSQL and target is SQLite
    if not args.source.startswith("postgresql://"):
        logger.error("Source database must be PostgreSQL (postgresql://...)")
        sys.exit(1)

    if not args.target.startswith("sqlite://"):
        logger.error("Target database must be SQLite (sqlite://...)")
        sys.exit(1)

    # Create migrator
    migrator = PostgreSQLToSQLiteMigrator(
