...
def save_recommendation_data(
    data: List[Tuple[str, str, float, int]],
    output_path: str
):
    """
...
Examples:
  ...
  python backend/scripts/load_external_data.py \\
    ...
    parser.add_argument(
        '--data-path',
        help='Path to raw dataset (required for arxiv, movielens)'
    )
    ...
    try:
        ...
        logger.info("  2. Train model with: python backend/scripts/train_classification.py --data-path " + args.output)
