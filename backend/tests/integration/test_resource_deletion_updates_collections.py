...
def test_resource_deletion_emits_event(db_session):
    ...
    resource = create_pending_resource(
        db_session,
        {"url": "https://example.com/test", "title": "Test Resource"}
    )
...
def test_resource_deleted_hook_queues_collection_update(db_session):
    ...
    with patch('backend.app.events.hooks.update_collection_embeddings_task') as mock_task:
        # Create event
        event = Event(
            name=SystemEvent.RESOURCE_DELETED,
