...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('parent_code', sa.String(length=3), sa.ForeignKey('classification_codes.code'), nullable=True),
        sa.Column('keywords', sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text()), 'postgresql'), nullable=False, server_default='[]'),
    )

    # Seed top-level UDC-inspired codes
    seeds = [
        ('000', 'Computer Science, Information & General Works', 'General knowledge, computing, information science', None, [
            'programming','software','coding','developer','python','java','javascript','typescript','c++','c#','go','rust','algorithm','data structure','computer','computing','information'
        ]),
        ('100', 'Philosophy & Psychology', None, None, []),
        ('200', 'Religion & Theology', None, None, []),
        ('300', 'Social Sciences', None, None, []),
        ('400', 'Language & Linguistics', 'Languages, linguistics, grammar and related topics', None, [
            'language','linguistics','grammar','vocabulary','pronunciation','syntax','semantics','phonetics','morphology'
        ]),
        ('500', 'Pure Sciences', 'Mathematics, physics, chemistry, biology, etc.', None, [
            'science','physics','chemistry','biology','mathematics','math','algebra','calculus','astronomy','geology','ecology','zoology','botany'
        ]),
        ('600', 'Technology & Applied Sciences', None, None, []),
        ('700', 'Arts & Recreation', None, None, []),
        ('800', 'Literature', None, None, []),
        ('900', 'History & Geography', 'Historical events, figures, and geography', None, [
            'history','ancient','medieval','renaissance','empire','revolution','napoleon','rome','greece','wwi','wwii','cold war','geography'
        ]),
    ]
