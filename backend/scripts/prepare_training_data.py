    ...
    for idx, sample in enumerate(samples):
        try:
            ...
            if not taxonomy_ids or not isinstance(taxonomy_ids, list):
                logger.warning(
                    f"Sample {idx}: Missing or invalid 'taxonomy_node_ids' field"
                )
                invalid_count += 1
                continue
...
def load_recommendation_test_data(
    ...
    logger.info(
        f"Found {len(user_id_map)} unique users and {len(item_id_map)} unique items"
    )
...
def augment_classification_data(
    data: List[Tuple[str, List[str]]], target_size: int = 500, multiplier: int = 3
) -> List[Tuple[str, List[str]]]:
    """
    ...
    logger.info(
        f"Augmenting dataset from {len(data)} to at least {target_size} samples"
    )
    ...
    try:
        if data_type == "classification":
            ...
            if not isinstance(data, tuple) or len(data) != 3:
                logger.error(
                    "Recommendation data must be a tuple of (interactions, user_map, item_map)"
                )
                return False
            ...
            for idx, interaction in enumerate(interactions[:5]):
                ...
                for field in required_fields:
                    if field not in interaction:
                        logger.error(
                            f"Interaction {idx}: Missing required field '{field}'"
                        )
                        return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        synth_interactions, synth_users, synth_items = create_synthetic_interactions(
            num_users=20, num_items=50, num_interactions=200
        )
