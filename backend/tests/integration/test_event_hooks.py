...
class TestHookRegistration:
    ...
    def test_hooks_registered_for_correct_events(self):
        ...
        assert len(event_emitter.get_listeners(SystemEvent.RESOURCE_CONTENT_CHANGED)) > 0
        assert len(event_emitter.get_listeners(SystemEvent.RESOURCE_METADATA_CHANGED)) > 0
        assert len(event_emitter.get_listeners(SystemEvent.RESOURCE_UPDATED)) >= 2  # Search + cache
        assert len(event_emitter.get_listeners(SystemEvent.CITATIONS_EXTRACTED)) > 0
        assert len(event_emitter.get_listeners(SystemEvent.USER_INTERACTION_TRACKED)) > 0
