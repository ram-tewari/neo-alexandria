    ...
    def generate(
        ...
        **kwargs
        ...
        try:
            ...
            candidates = self.db.query(Resource).filter(
                Resource.embedding.isnot(None),
                ~Resource.id.in_(interacted_ids)
            ).all()
    ...
    def _build_user_profile(
        ...
        for interaction in interactions:
            resource = self.db.query(Resource).filter(
                Resource.id == interaction.resource_id
            ).first()
...
class GraphBasedStrategy(RecommendationStrategy):
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        logger.info(
            f"Generating graph-based recommendations "
            f"for user {user_id}, limit={limit}"
        )
        ...
        try:
            ...
            candidates = [
                (rid, score) for rid, score in neighbors.items()
            ...
            logger.info(
                f"Generated {len(recommendations)} graph-based recommendations"
            )
            return recommendations
    ...
    def _find_graph_neighbors(
        ...
        for seed_id in seed_ids:
            ...
            outgoing = self.db.query(Citation).filter(
                Citation.source_resource_id == seed_id
            ).all()
            ...
            incoming = self.db.query(Citation).filter(
                Citation.target_resource_id == seed_id
            ).all()
    ...
    def _weighted_fusion(
        ...
        for recommendations, weight in strategy_results:
            for rec in recommendations:
                ...
                resource_scores[resource_id]['strategies'].append(
                    rec.strategy
                )
        ...
        for rank, (resource_id, scores) in enumerate(sorted_resources[:limit], start=1):
            ...
            combined_reason = (
                ...
                else "Recommended by multiple strategies"
            )
            
            # Create fused recommendation
            rec = self._create_recommendation(
                resource_id=resource_id,
                user_id=list(strategy_results[0][0])[0].user_id if strategy_results[0][0] else "",
