...
def update_imports(content: str) -> str:
    ...
    content = content.replace(
        "from ..database.models import", "from app.database.models import"
    )

    # Update schema imports to use module-local
    content = content.replace(
        "from ..schemas.graph import", "from app.modules.graph.schema import"
    )
    content = content.replace(
        "from ..schemas.discovery import", "from app.modules.graph.schema import"
    )

    # Update shared kernel imports
    content = content.replace(
        "from ..shared.base_model import Base", "from app.shared.base_model import Base"
    )
    content = content.replace(
