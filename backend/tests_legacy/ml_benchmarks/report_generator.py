...
class ReportGenerator:
    ...
    def __init__(
        self, results: Dict[str, BenchmarkSuiteResult], output_dir: str = "docs"
    ):
        """
    ...
    def _generate_header(self) -> str:
        ...
        try:
            ...
            lines.append(
                f"**Hardware**: {platform.processor() or 'Unknown CPU'}, "
    ...
    def _generate_executive_summary(self) -> str:
        ...
        if total_failed == 0:
            ...
            pass_rate = (
                100 * (total_tests - total_failed) / total_tests
                if total_tests > 0
                else 0
            )
            lines.append(
                f"**Overall Status**: ⚠️ {total_failed}/{total_tests} tests failed ({pass_rate:.1f}% pass rate)"
            )
    ...
    def _generate_methodology(self) -> str:
        ...
        lines.append("")
        lines.append(
            "- **Classification**: 200 balanced samples across 10 taxonomy classes"
        )
        lines.append(
            "- **Recommendation**: 50 users, 200 items, 1000 interactions, 20% held-out"
        )
        lines.append(
            "- **Search**: 50 queries with graded relevance judgments (0-3 scale)"
        )
        lines.append(
        ...
        lines.append(
            "| Classification Latency | 100ms | 75ms | Real-time user experience requirement |"
        )
        lines.append("| NCF Latency | 50ms | 30ms | Fast recommendation generation |")
        lines.append(
            "| Search Latency | 200ms | 150ms | Acceptable search response time |"
        )
        lines.append(
            "| Embedding Latency | 500ms | 300ms | Batch processing acceptable |"
        )
    ...
    def _generate_current_results(self) -> str:
        ...
        lines.append("")
        lines.append(
            f"**Benchmark Date**: {self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append("")
    ...
    def _generate_classification_section(
        ...
        lines.append("")
        lines.append(
            f"**Status**: {'✅ PASS' if suite_result.failed_tests == 0 else '❌ FAIL'}"
        )
        lines.append(
    ...
    def _generate_summarization_section(
        self, suite_result: BenchmarkSuiteResult
    ) -> str:
        """
    ...
    def _generate_regressions(self) -> str:
        ...
        if not history_path.exists():
            lines.append(
                "No previous benchmark results found. This is the first benchmark run."
            )
            lines.append("")
        ...
        try:
            ...
            if not regressions:
                ...
                lines.append("")
                lines.append(
                    "All metrics are stable or improved compared to the previous run."
                )
            else:
                ...
                lines.append(
                    "| Suite | Metric | Previous | Current | Change | Severity |"
                )
                lines.append(
                    "|-------|--------|----------|---------|--------|----------|"
                )
    ...
    def _extract_metric_value(
        self, suite_result: BenchmarkSuiteResult, metric_name: str
    ) -> Optional[float]:
        """
    ...
    def _generate_recommendations(self) -> str:
        ...
        for suite_name, suite_result in self.results.items():
            if suite_result.failed_tests > 0:
                ...
                recommendations = self._generate_suite_specific_recommendations(
                    suite_name, suite_result, priority="high"
                )
                ...
                recommendations = self._generate_suite_specific_recommendations(
                    suite_name, suite_result, priority="medium"
                )
                medium_priority.extend(recommendations)
            else:
                # Low priority: optimization opportunities
                recommendations = self._generate_suite_specific_recommendations(
                    suite_name, suite_result, priority="low"
                )
        ...
        if not high_priority and not medium_priority and not low_priority:
            ...
            lines.append("")
            lines.append(
                "All benchmarks are performing well. Continue monitoring for regressions."
            )
            return "\n".join(lines)
    ...
    def _generate_suite_specific_recommendations(
        self, suite_name: str, suite_result: BenchmarkSuiteResult, priority: str
    ) -> List[Dict[str, str]]:
        """
    ...
    def _generate_reproduction_steps(self) -> str:
        ...
        lines.append(
            "python backend/tests/ml_benchmarks/benchmark_runner.py --suite classification"
        )
        lines.append("")
        ...
        lines.append(
            "- **Solution**: Download model checkpoints or train models locally"
        )
        lines.append("")
        lines.append("**Issue**: Out of memory errors")
        lines.append(
            "- **Solution**: Reduce batch sizes in test configuration or run suites individually"
        )
        lines.append("")
        lines.append("**Issue**: Tests timeout")
        lines.append(
            "- **Solution**: Increase timeout in pytest.ini or run on faster hardware"
        )
    ...
    def _save_to_history(self) -> None:
        ...
        try:
            ...
            if history_path.exists():
                ...
                history = {"created_at": self.timestamp.isoformat(), "runs": []}

            # Create current run entry
            current_run = {"timestamp": self.timestamp.isoformat(), "suites": {}}
...
def main():
    ...
    parser.add_argument(
        "--results", type=str, required=True, help="Path to benchmark results JSON file"
    )
