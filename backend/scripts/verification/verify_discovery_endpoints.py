...
def verify_discovery_endpoints():
    ...
    response = client.get(
        ...
        params={"hops": 2, "min_weight": 0.0, "limit": 50},
    )
    print(f"   Status code: {response.status_code}")
    ...
    response = client.get("/discovery/hypotheses", params={"skip": 0, "limit": 10})
    print(f"   Status code: {response.status_code}")
    ...
    response = client.post(
        ...
        json={"is_valid": True, "notes": "Test validation"},
    )
    print(f"   Status code: {response.status_code}")
    if response.status_code == 404:
    ...
    response = client.get(
        "/discovery/hypotheses",
        params={"hypothesis_type": "invalid_type", "skip": 0, "limit": 10},
    )
    print(f"   Status code: {response.status_code}")
