...
def create_deep_taxonomy_tree(
    service: TaxonomyService, depth: int = 5, breadth: int = 3, prefix: str = None
) -> List[TaxonomyNode]:
    """
    ...
    def create_children(parent: TaxonomyNode, current_depth: int):
        ...
        for i in range(breadth):
            child = service.create_node(
                name=f"{parent.name} - Child {i}", parent_id=parent.id
            )
            nodes.append(child)
...
def test_tree_retrieval_performance_full_depth(test_db):
    ...
    assert avg_time < 200.0, (
        f"Full tree retrieval took {avg_time:.2f}ms, expected <200ms"
    )
...
def test_single_prediction_inference_time(test_db):
    ...
    try:
        ...
        assert avg_time < 100.0, (
            f"Single prediction took {avg_time:.2f}ms, expected <100ms"
        )
...
def test_batch_prediction_performance(test_db):
    ...
    try:
        ...
        for batch_size in batch_sizes:
            ...
            times = []
            for _ in range(5):
                exec_time, predictions = measure_time(
                    service.predict_batch, texts, top_k=5
                )
                times.append(exec_time)
...
def test_ancestor_query_scalability(test_db):
    ...
    for depth, exec_time, count in results:
        assert exec_time < 10.0, (
            f"Query at depth {depth} took {exec_time:.2f}ms, expected <10ms"
        )
...
def test_descendant_query_scalability(test_db):
    ...
    for idx, (depth, breadth) in enumerate(configs):
        ...
        nodes = create_deep_taxonomy_tree(
            service, depth=depth, breadth=breadth, prefix=f"ScaleTest{idx}"
        )
        root = nodes[0]
        ...
        print(
            f"Depth {depth}, Breadth {breadth}: {exec_time:.2f}ms ({len(descendants)} descendants)"
        )

    print("\nDescendant query scalability:")
    for depth, breadth, exec_time, count in results:
        print(
            f"  Depth {depth}, Breadth {breadth}: {exec_time:.2f}ms ({count} descendants)"
        )

    # All queries should be under 10ms
    for depth, breadth, exec_time, count in results:
        assert exec_time < 10.0, (
            f"Query (depth={depth}, breadth={breadth}) took {exec_time:.2f}ms, expected <10ms"
        )
...
def test_performance_summary(test_db):
    ...
    nodes = create_deep_taxonomy_tree(
        taxonomy_service, depth=4, breadth=2, prefix="SummaryTest"
    )
    root = nodes[0]
    deepest = nodes[-1]
