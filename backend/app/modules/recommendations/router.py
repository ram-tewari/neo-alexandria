...
@recommendations_router.get("/simple", response_model=RecommendationResponse, status_code=status.HTTP_200_OK)
def get_recommendations_simple(limit: int = Query(10, ge=1, le=100), db: Session = Depends(get_sync_db)):
    """
...
@recommendations_router.post("/interactions", response_model=InteractionResponse, status_code=status.HTTP_201_CREATED)
...
@recommendations_router.post("/feedback", response_model=FeedbackResponse, status_code=status.HTTP_201_CREATED)
