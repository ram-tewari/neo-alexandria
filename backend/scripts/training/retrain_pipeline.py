...
class RetrainingPipeline:
    ...
    def _load_config(self) -> Dict:
        ...
        logger.info(f"Data growth threshold: {config.get('data_growth_threshold', 0.10)}")
    ...
    def _get_last_training_date(self) -> str:
        ...
        try:
            ...
            if not registry_path.exists():
                ...
                logger.warning(f"No version registry found, using default date: {default_date}")
                return default_date
            ...
            if not versions:
                default_date = self.config.get("default_start_date", "2020-01-01")
                logger.warning(f"No versions in registry, using default date: {default_date}")
                return default_date
            ...
            if created_at:
                ...
                logger.info(f"Last training date from version {latest_version.get('version')}: {date_str}")
                return date_str
            else:
                default_date = self.config.get("default_start_date", "2020-01-01")
                logger.warning(f"No created_at in latest version, using default date: {default_date}")
                return default_date
    ...
    def augment_dataset(
        ...
        augmented_dir = self.splits_dir.parent / f"arxiv_classification_augmented_{timestamp}"
        ...
        train_samples, val_samples, test_samples = self.preprocessor.create_train_val_test_split(
    ...
    def evaluate_improvement(
        ...
        accuracy_improvement = new_metrics.get('accuracy', 0) - prod_metrics.get('accuracy', 0)
        ...
        logger.info(f"F1 improvement: {f1_improvement:+.4f} ({f1_improvement*100:+.2f}%)")
    ...
    def promote_if_better(
        ...
        logger.info(f"Accuracy improvement: {accuracy_improvement:+.4f} ({accuracy_improvement*100:+.2f}%)")
        logger.info(f"Promotion threshold: {promotion_threshold:.4f} ({promotion_threshold*100:.2f}%)")
    ...
    def send_notification(
        ...
        if results:
            ...
            if "dataset_size" in results:
                ...
                notification_text += f"  Growth: {results.get('new_data_count', 0)} new papers\n"
            
            # Model versions
            if "new_version" in results:
                notification_text += "\nModel Versions:\n"
                notification_text += f"  Production: {results.get('production_version', 'N/A')}\n"
    ...
    def run_pipeline(self, trigger_type: str = "scheduled") -> Dict:
        ...
        try:
            ...
            if not existing_train_file.exists():
                ...
                raise FileNotFoundError(f"Training data not found: {existing_train_file}")
            ...
            results["training_time_seconds"] = training_metrics.get("training_time_seconds", 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Check for new data without training"
    )
