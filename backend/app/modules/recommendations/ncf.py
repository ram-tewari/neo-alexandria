...
class NCFService:
    ...
    def __init__(
        ...
        logger.info(
            f"NCFService initialized with model_path={self.model_path}"
        )
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            self.user_idx_to_id = {idx: user_id for user_id, idx in self.user_id_map.items()}
            self.item_idx_to_id = {idx: item_id for item_id, idx in self.item_id_map.items()}
    ...
    def recommend(
        ...
        if exclude_seen:
            seen_items = self.db.query(UserInteraction.resource_id).filter(
