...
class ClassificationService:
        ...
        **Validates: Requirements 10.1, 10.2, 10.3, 10.4, 10.5**
        ...
        if use_rules:
            try:
                ...
                logger.info(
                    f"Rule-based classification: {len(rule_predictions)} predictions"
                )
            except Exception as e:
        ...
        **Validates: Requirements 10.2, 10.3**
        ...
        for pred in predictions:
            node_scores[pred["node_id"]].append(
                {"confidence": pred["confidence"], "method": pred["method"]}
            )
        ...
        return {
            ...
            "alternatives": final_predictions[1:5]
            if len(final_predictions) > 1
            else [],
            "all_predictions": final_predictions,
        }

    def _apply_classification(self, resource_id: str, classification: Dict) -> None:
        """
        ...
        **Validates: Requirement 10.4**
        ...
        resource_taxonomy = (
            self.db.query(ResourceTaxonomy).filter_by(resource_id=resource_id).first()
        )
    ...
    def batch_classify(
        self, resource_ids: List[str], use_ml: bool = True, use_rules: bool = True
    ) -> Dict[str, Dict]:
        """
        ...
        for resource_id in resource_ids:
            try:
                ...
                results[resource_id] = {"error": str(e)}
    ...
    def get_classification(self, resource_id: str) -> Optional[Dict]:
        ...
        node = (
            self.db.query(TaxonomyNode).filter_by(id=resource.taxonomy_node_id).first()
        )
    ...
    def reclassify_uncertain(self, threshold: float = 0.5, limit: int = 100) -> Dict:
        """
        ...
        return {"total": len(uncertain_ids), "results": results}
